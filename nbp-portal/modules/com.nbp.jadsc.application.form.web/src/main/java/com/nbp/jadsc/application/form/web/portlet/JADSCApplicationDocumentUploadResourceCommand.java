...
@Component(immediate = true, property = {
		"javax.portlet.name="
				 + JADSCApplicationFormPortletKeys.JADSCAPPLICATIONFORM,
		"mvc.command.name=jadsc/Doucuments/Upload" }, service = MVCResourceCommand.class)
public class JADSCApplicationDocumentUploadResourceCommand implements MVCResourceCommand {
	...
	private static Log _log = LogFactoryUtil.getLog(JADSCApplicationDocumentUploadResourceCommand.class.getName());

	@Override
	public boolean serveResource(ResourceRequest resourceRequest, ResourceResponse resourceResponse)
			throws PortletException {
		_log.info("[SUPPORTING DOC UPLOAD START]");
		...
		try {
			...
			if (Validator.isNotNull(inputStream)) {
				...
				DLFolder dlFolder = createFolder(themeDisplay.getUserId(), jadscApplicationId,
				...
				resourceRequest.getPortletContext()
						.getRequestDispatcher("/supporting-documents/supporting-document-info.jsp")
	...
	private DLFolder createFolder(long userId, long factoriesRegistrationApplicationId, long groupId) {
		...
		try {
			...
			try {
				dlFolder = DLFolderLocalServiceUtil.getFolder(repositoryId, rootFolder.getFolderId(),
						String.valueOf(factoriesRegistrationApplicationId));
			} catch (Exception e) {
			...
			if (Validator.isNull(dlFolder)) {
				dlFolder = DLFolderLocalServiceUtil.addFolder(String.valueOf(CounterLocalServiceUtil.increment()), userId, groupId, repositoryId, false,
						rootFolder.getFolderId(), String.valueOf(factoriesRegistrationApplicationId),
						ROOT_FOLDER_DESCRIPTION, false, serviceContext);
			}

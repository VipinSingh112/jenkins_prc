...
<%@ include file="/forms/factories-popup.jsp" %>
<%@ include file="/factories-registration-wizard.jsp" %>
<jsp:include page="/js/product-js.jsp"/>
            ...
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <p class="modal-subtitle f2">Your application for Factories Registration has been correctly saved as draft.</p>
                            ...
                            <div class="row">
                                <div class="col-lg-5 d-flex justify-content-center">
                                    <div class="licence-box ml-2">
                                        <div class="left">
											<svg width="36" height="42" viewBox="0 0 205 220" fill="none" xmlns="http://www.w3.org/2000/svg"> 
                                        ...
                                        <div class="right pl-2">
                                            <p class="licence-title f2">The Food Establishment<br> Transaction Number</p>
...
<script>
var establishmentSignaturefactoriesImgStatus = "";
var processedFoodSignaturefactoriesImgStatus = "";
...
function showSelectedCategory(){
	...
	areYouProcessing="<%=areYouProcessing%>";
	...
	sizeOfProductDetail="<%=sizeOfProductDetail%>";
	
	$("#factoriesRegistrationApplicationId").val(<%=factoriesApplicationId%>);
	$("#factoriesTransactionType").val(typeOfTransaction);
	$("#factoriesApplicationTypeVal").val(productCategories);
	$("#paymentMethod").val(paymentMethod);
	$("#amountCurrency").val(currency);

	
	if(sizeOfProductDetail>0){
		var counterOfProductDetail=parseInt(sizeOfProductDetail)+1;
		$(".addProductDetailTableBox").removeClass("hide");
		$("#productDetail").val(counterOfProductDetail);
		let currentValue = parseInt($("#productDetailVal").val(), 10) || 0; // Use 0 if the value is empty or NaN
		$("#productDetailVal").val(currentValue + 1);

	}
	/* Type of Applicant */
	if (typeOfTransaction == "New Application"){
	$("#factoriesRegistration .wizard-title span").each( function( index, element ){
	    if($(this).text() == "New Application"){
	    	$(this).closest(".wizard-card").addClass("active");
	    	$(this).closest("a").click();
	    	}
		  });
	
	$("#factoriesRegistrationProcessingType .wizard-title").each( function( index, element ){
	    if($(this).text() == areYouProcessing){
	    	$(this).closest(".wizard-card").addClass("active");
	    	$(this).closest("a").click();
	       }
	});
	$("#processedActDescription").val(productCategories);
	 if(productCategories !=""){
	    	var arrayOfSelected = productCategories.split(";");
	    	
	    	var categoryToElementMap = {
	    	        "Low Acid Canned Foods": "#lowAcidCanned",
	    	        "Acidified Foods": "#acidifiedFoods",
	    	        "Processed Ackees":"#processedAckees",
	    	        "Packaged Water":"#packagedWater",
	    	        "Sauces":"#sauces",
	    	        
	    	        "Sugar":"#sugar",
	    	        "Coconut Water":"#coconutWater",
	    	        "Juices":"#juices",
	    	        "Preserves (Jams Jellies, Marmalades)":"#preserves",
	    	        "Roots Wine/ Herbal Wine":"#rootsWine",
	    	        
	    	        "Baked Products":"#bakedProducts",
	    	        "Pastries":"#pastries",
	    	        "Meat & Meat Products":"#meatProducts",
	    	        "Honey":"#honey",
	    	        "Nuts":"#nuts",
	    	        
	    	        "Fat & Oils":"#oils",
	    	        "Margarines & Butter":"#margarines",
	    	        "Coffee":"#coffee",
	    	        "Confectionaries":"#confectionaries",
	    	        "Local Snacks":"#localSnacks"
	    	    };
	    	
	    	for (var category in categoryToElementMap) {
	            if (arrayOfSelected.includes(category)) {
	...
	}else if (typeOfTransaction == "Re-Registration"){
		$("#factoriesReRegistration .wizard-title span").each( function( index, element ){
		    if($(this).text() == "Re-Registration"){
		    	$(this).closest(".wizard-card").addClass("active");
		    	}
		   
			  });
	}
	if (typeOfTransaction == "Re-Registration"){
		/*   $("#factoriesTransactionType").addClass("hide");
		  $(".instruction-div").addClass("hide");
		  $("#factoriesRegistration").click(); */
		$("#factoriesRegistration").click(); 
	}

	if (areYouProcessing=="Prescribed Foods") {
		$("#payment_category").html("Prescribed Foods");
	    $("#standardsActTab").removeClass("hide");
		$("#processedActTab").removeClass("hide");  	    
	   } 
    else if (areYouProcessing=="Non-Prescribed Foods") {
    	$("#payment_category").html("Non-Prescribed Foods");
	    $("#processedActTab").addClass("hide");
		$("#standardsActTab").removeClass("hide");
	}
	/* Product Categories */
	
	 $("input[name='establishmentBefore']:checked").each(function () {
		    if($(this).val() == "Yes"){
             $("#other-copy, #other-copy1").removeClass('hide');
			   }else if($(this).val() == "No"){
             $("#other-copy, #other-copy1").addClass('hide');
			   }
			});
	 $("input[name='establishmentOperated']:checked").each(function () {
		    if($(this).val() == "Yes"){
             $("#other-field, #other-field1").removeClass('hide');
			   }else if($(this).val() == "No"){
             $("#other-field, #other-field1").addClass('hide');
			   }
			});

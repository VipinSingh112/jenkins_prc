...
@Component(immediate = true, property = {
		...
		"mvc.command.name=factories/Multiple/Document/Upload" }, service = MVCResourceCommand.class)
public class FactoriesRegistrationDocumentsResourceCommand implements MVCResourceCommand {
	...
	private static Log _log = LogFactoryUtil.getLog(FactoriesRegistrationDocumentsResourceCommand.class);
	...
	public boolean serveResource(ResourceRequest resourceRequest, ResourceResponse resourceResponse)
		...
		long factoriesRegistrationApplicationId = ParamUtil.getLong(resourceRequest,"factoriesRegistrationApplicationId");
		...
		if (folderName.equals("Generic Document Upload")) {
		    ...
		    try {
		        ...
		        try {
		            subDlFolder = DLFolderLocalServiceUtil.getFolder(
		                    themeDisplay.getScopeGroupId(), dlFolder.getFolderId(), "Generic Document Upload");
		        } catch (Exception e) {
		...
		if (Validator.isNotNull(fileList) && fileList.length > 0) {
			dlFolder = FactoriesRegistrationApplicationUtil.createSubFolder(themeDisplay.getUserId(),
					factoriesRegistrationApplicationId, themeDisplay.getScopeGroupId(), folderName,

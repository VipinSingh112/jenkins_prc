...
(immediate = true, property = {
		...
		"mvc.command.name=/osi/services/download/pdf" 
		}, service = MVCResourceCommand.class)

public class OSIServicesDownloadPDFResourceCommand implements MVCResourceCommand{
	private static Log _log = LogFactoryUtil.getLog(OSIServicesDownloadPDFResourceCommand.class);
	...
	public boolean serveResource(ResourceRequest resourceRequest, ResourceResponse resourceResponse)
		...
		try {
			String pdfTemplate = commonsUtil.replaceOsiServicesPdfVar(osiServicesApplication.getDoYouWant(), osiServicesApplication.getNatureOfApplicant(), osiServiceApplicationId, themeDisplay, checkPdfDownloadWithoutDocument);
//		 it converted template into file
			File file = commonsUtil.getPreviewFileDownloadPdf(resourceRequest, resourceResponse,
					"osi-company", pdfTemplate);
			String fileNameContentDisp = "OsiServicesApplicationPDF.pdf";
			...
			serviceContext = ServiceContextFactory.getInstance(DLFileEntry.class.getName(), resourceRequest);
			  DLFolder dlFolder = createFolder(themeDisplay.getUserId(), osiServiceApplicationId, themeDisplay.getScopeGroupId()); 
			  DLFolder osiAppDlFolder=null;
			  osiAppDlFolder= OSIServiceApplicationUtil.createSubFolder(themeDisplay.getUserId(), osiServiceApplicationId, themeDisplay.getScopeGroupId(),
					  "OSI Services Application Form PDF", "OSI Services Application Form PDF");
			commonsUtil.updateFolderPermission(osiAppDlFolder);
			...
			try {
			entry=DLAppLocalServiceUtil.getFileEntryByFileName(themeDisplay.getScopeGroupId(), osiAppDlFolder.getFolderId(), fileNameContentDisp);
			...
			if(entry==null) {
			try {
				docFileEntry=	_dlAppService.addFileEntry("OsiServicesApplicationPDF" + osiServiceApplicationId, osiAppDlFolder.getRepositoryId(),
						osiAppDlFolder.getFolderId(), fileNameContentDisp, mimeType, title, null, StringPool.BLANK,
				...
				dlFileEntry = DLFileEntryLocalServiceUtil.getDLFileEntryByExternalReferenceCode("OsiServicesApplicationPDF" + osiServiceApplicationId,
						osiServicesApplication.getGroupId());
				docFileEntry = DLAppLocalServiceUtil.getFileEntry(dlFileEntry.getFileEntryId());
				osiServicesApplication.setPdfFileEntryId(dlFileEntry.getFileEntryId());
	...
	private DLFolder createFolder(long userId, long osiServiceApplicationId, long groupId) {
		...
		try {
			...
			if (Validator.isNull(dlFolder)) {
				dlFolder = DLFolderLocalServiceUtil.addFolder(String.valueOf(CounterLocalServiceUtil.increment()), userId, groupId, repositoryId, false,
						rootFolder.getFolderId(), String.valueOf(osiServiceApplicationId), ROOT_FOLDER_DESCRIPTION, false,
						serviceContext);
			}else {
				 dlFolder=DLFolderLocalServiceUtil.getDLFolder(osiServiceApplicationId);
			}
		} catch (PortalException e1) {

...
@Component(immediate = true, property = {
		...
		"mvc.command.name=/osi/services/documents/upload" }, service = MVCResourceCommand.class)
public class OsiServicesDocumentUploadResourceCommand implements MVCResourceCommand {
	...
	private static Log _log = LogFactoryUtil.getLog(OsiServicesDocumentUploadResourceCommand.class.getName());
	...
	public boolean serveResource(ResourceRequest resourceRequest, ResourceResponse resourceResponse)
		...
		try {
			...
			if (Validator.isNotNull(inputStream)) {
				...
				DLFolder dlFolder = createFolder(themeDisplay.getUserId(), osiServiceApplicationId,
				 ...
				 * OSIServicesApplicationFormPortletKeys.DOC_NO); if (docNoExpandoColumn ==
				 * null) { docNoExpandoColumn =
				 * ExpandoColumnLocalServiceUtil.getColumn(themeDisplay.getCompanyId(),
				 * classNameId, expandoTable.getName(),
				 * OSIServicesApplicationFormPortletKeys.DOC_NO); }
		...
		try {
			...
			if (Validator.isNull(dlFolder)) {
				dlFolder = DLFolderLocalServiceUtil.addFolder(String.valueOf(CounterLocalServiceUtil.increment()), userId, groupId, repositoryId, false,
						rootFolder.getFolderId(), String.valueOf(osiServiceApplicationId), ROOT_FOLDER_DESCRIPTION,

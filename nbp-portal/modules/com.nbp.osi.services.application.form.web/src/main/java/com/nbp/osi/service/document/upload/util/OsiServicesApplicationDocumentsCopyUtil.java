...
public class OsiServicesApplicationDocumentsCopyUtil {
	private static Log _log = LogFactoryUtil.getLog(OsiServicesApplicationDocumentsCopyUtil.class.getName());

	public synchronized static void copyFolder(long repositoryId, long sourceFolderId, long parentFolderId, String name,
			String description, ServiceContext serviceContext, long folderId, long userId,
			long osiServiceApplicationId) {
		...
		try {
			destFolder = DLAppServiceUtil.addFolder(String.valueOf(CounterLocalServiceUtil.increment()), repositoryId,
					parentFolderId, String.valueOf(osiServiceApplicationId), "Document Folder", serviceContext);
		} catch (PortalException e) {
			_log.error("Error*******" + e.getMessage());
		}

		try {
			copyFolder(sourceDlFolder, destFolder, serviceContext, userId, repositoryId, osiServiceApplicationId);
		} catch (PortalException e) {
	...
	protected synchronized static void copyFolder(Folder srcFolder, Folder destFolder, ServiceContext serviceContext, long userId,
			long groupId, long osiServiceApplicationId) throws PortalException {
	...
	public synchronized static DLFileEntry copyFileEntry(long userId, long groupId, long repositoryId, long fileEntryId,
	...
	private synchronized static InputStream _getInputStream(DLFileEntry dlFileEntry, DLFileVersion dlFileVersion)
	...
	public synchronized static void copyFileEntryMetadata(long companyId, long fileEntryTypeId, long fileEntryId,
	...
	private synchronized static void _copyFileEntryMetadata(long companyId, long fileEntryId, long fromFileVersionId,

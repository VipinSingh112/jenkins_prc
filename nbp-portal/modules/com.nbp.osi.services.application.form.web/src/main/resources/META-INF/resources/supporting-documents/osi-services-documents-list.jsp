...
supportingDocumentArray=(JSONObject)request.getAttribute("documentListJsonArray");
 ...
 if(Validator.isNotNull(request.getAttribute("osiServiceApplicationId"))){
	 osiServiceId= String.valueOf(request.getAttribute("osiServiceApplicationId"));
} 
...
DLFolder supprtingDocMainFolder=null;
DLFolder supportingDocumentsAppIdFolder =null;
try {
	supprtingDocMainFolder = DLFolderLocalServiceUtil.getFolder(themeDisplay.getScopeGroupId(),
			DLFolderConstants.DEFAULT_PARENT_FOLDER_ID, "OSI Services Supported Documents");
	supportingDocumentsAppIdFolder = DLFolderLocalServiceUtil.getFolder(themeDisplay.getScopeGroupId(),
			supprtingDocMainFolder.getFolderId(),
	  String.valueOf(osiServiceId));
...
for(String documentNames:documentNameArray){
	...
	try {
		fileEntry = DLAppLocalServiceUtil.getFileEntry(themeDisplay.getScopeGroupId(),
				supportingDocumentsAppIdFolder.getFolderId(), documentNames.trim());
	} catch (Exception e) {
		e.printStackTrace();
		e.getCause();
		e.getMessage();
...
for(String documentNamesNR:notRequireddocumentNameArray){
	...
	try {
		fileEntryNR = DLAppLocalServiceUtil.getFileEntry(themeDisplay.getScopeGroupId(),
				supportingDocumentsAppIdFolder.getFolderId(), documentNamesNR.trim());

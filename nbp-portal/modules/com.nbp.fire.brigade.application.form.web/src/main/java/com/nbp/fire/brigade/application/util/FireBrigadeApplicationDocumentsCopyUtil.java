...
public class FireBrigadeApplicationDocumentsCopyUtil {
	private static Log _log = LogFactoryUtil.getLog(FireBrigadeApplicationDocumentsCopyUtil.class.getName());
	...
	public static void copyFolder(long repositoryId, long sourceFolderId, long parentFolderId, String name,
		...
		try {
			destFolder = DLAppServiceUtil.addFolder(String.valueOf(CounterLocalServiceUtil.increment()),repositoryId,
					parentFolderId, String.valueOf(fireBrigadeApplicationId), "Document Folder", serviceContext);
		} catch (PortalException e) {
			_log.info("Error*******" + e.getMessage());
		}

		try {
			copyFolder(sourceDlFolder, destFolder, serviceContext, userId, repositoryId, fireBrigadeApplicationId);
		} catch (PortalException e) {
	...
	protected static void copyFolder(Folder srcFolder, Folder destFolder, ServiceContext serviceContext, long userId,
			long groupId, long fireBrigadeApplicationId) throws PortalException {
		...
		for (FileEntry srcFileEntry : srcFileEntries) {
			try {
				...
				_log.info("Error*******" + exception.getMessage());
			}
		}
	...
	public static DLFileEntry copyFileEntry(long userId, long groupId, long repositoryId, long fileEntryId,
		...
		try {
		newDLFileEntry=	DLFileEntryLocalServiceUtil.copyFileEntry(dlFileEntry.getUserId(), dlFileEntry.getGroupId(), dlFileEntry.getRepositoryId(), 
				dlFileEntry.getFileEntryId(), destFolderId, dlFileEntry.getFileName(), serviceContext);
	...
	private static InputStream _getInputStream(DLFileEntry dlFileEntry, DLFileVersion dlFileVersion)
		...
		try {
			return DLFileEntryLocalServiceUtil.getFileAsStream(
				    dlFileEntry.getFileEntryId(), dlFileVersion.getVersion());

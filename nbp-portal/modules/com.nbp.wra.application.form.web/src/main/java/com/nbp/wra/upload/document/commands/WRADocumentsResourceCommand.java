...
@Component(immediate = true, property = {
		...
		"mvc.command.name=/wra/signature/upload"
		},
		service = MVCResourceCommand.class
		)
public class WRADocumentsResourceCommand implements MVCResourceCommand {
	private static long PARENT_FOLDER_ID = DLFolderConstants.DEFAULT_PARENT_FOLDER_ID;
	private static String ROOT_FOLDER_NAME = "WRA Supported Documents";
	private static Log _log = LogFactoryUtil.getLog(WRADocumentsResourceCommand.class);
@Override
	...
	if (folderName.equals("Generic Documents")) {
		...
		if (Validator.isNotNull(fileList) && fileList.length > 0) {
			dlFolder = WRAApplicationUtil.createSubFolder(
					themeDisplay.getUserId(), wraApplicationId, themeDisplay.getScopeGroupId(), folderName,
					"WRA Supported Documents");
				
				 if(!folderName.equals("Abstract Renew Signature Applicant")||!folderName.equals("Abstract Signature Applicant")||!folderName.equals("Permit Signature Applicant")
						  ||!folderName.equals("Well Drilling Signature Applicant")) { 
					  try {

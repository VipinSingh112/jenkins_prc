...
"mvc.command.name=/hsra/download/pdf" }, service = MVCResourceCommand.class)

public class HsraDownloadPdfResourceCommand implements MVCResourceCommand{
	private static Log _log = LogFactoryUtil.getLog(HsraDownloadPdfResourceCommand.class);
	...
	public boolean serveResource(ResourceRequest resourceRequest, ResourceResponse resourceResponse)
		...
		System.out.println("----------before replace hsra pdf var--------------------");
		try {
			String pdfTemplate = commonsUtil.replaceHsraApplicationPdfVar(hsraApplication.getTypeOfApplicant(), hsraApplicationId, themeDisplay, checkPdfDownloadWithoutDocument);
...
//		 it converted template into file
			...
			String fileNameContentDisp = "HsraApplicationPDF.pdf";
			...
			serviceContext = ServiceContextFactory.getInstance(DLFileEntry.class.getName(), resourceRequest);
			  DLFolder dlFolder = createFolder(themeDisplay.getUserId(), hsraApplicationId, themeDisplay.getScopeGroupId()); 
			  DLFolder hsraAppDlFolder=null;
			  hsraAppDlFolder= HSRAApplicationUtil.createSubFolder(themeDisplay.getUserId(), hsraApplicationId, themeDisplay.getScopeGroupId(),   "Hsra Application Form PDF", "Hsra Application Form PDF");
			commonsUtil.updateFolderPermission(hsraAppDlFolder);
			...
			try {
			entry=DLAppLocalServiceUtil.getFileEntryByFileName(themeDisplay.getScopeGroupId(), hsraAppDlFolder.getFolderId(), fileNameContentDisp);
			...
			if(entry==null) {
			try {
				docFileEntry=	_dlAppService.addFileEntry("HsraApplicationPDF" + hsraApplicationId, hsraAppDlFolder.getRepositoryId(),
						hsraAppDlFolder.getFolderId(), fileNameContentDisp, mimeType, title, null, StringPool.BLANK,
				...
				dlFileEntry = DLFileEntryLocalServiceUtil.getDLFileEntryByExternalReferenceCode("HsraApplicationPDF" + hsraApplicationId,
						hsraApplication.getGroupId());
				docFileEntry = DLAppLocalServiceUtil.getFileEntry(dlFileEntry.getFileEntryId());
				hsraApplication.setPdfFileEntryId(dlFileEntry.getFileEntryId());
		...
		try {
			...
			if (Validator.isNull(dlFolder)) {
				dlFolder = DLFolderLocalServiceUtil.addFolder(String.valueOf(CounterLocalServiceUtil.increment()), userId, groupId, repositoryId, false,
				 ...
				 dlFolder=DLFolderLocalServiceUtil.getDLFolder(hsraApplicationId);
			}
		} catch (PortalException e1) {

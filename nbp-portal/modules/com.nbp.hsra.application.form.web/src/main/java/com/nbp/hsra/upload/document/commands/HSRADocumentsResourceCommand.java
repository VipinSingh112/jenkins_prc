...
public class HSRADocumentsResourceCommand implements MVCResourceCommand {
	...
	private static Log _log = LogFactoryUtil.getLog(HSRADocumentsResourceCommand.class);

	@Override
	public synchronized boolean serveResource(ResourceRequest resourceRequest, ResourceResponse resourceResponse)
			throws PortletException {
		_log.info("File upload started HSRA-----------------------");
		...
		if (Validator.isNotNull(fileList) && fileList.length > 0) {
			dlFolder = HSRAApplicationUtil.createSubFolder(themeDisplay.getUserId(), hsraApplicationId, themeDisplay.getScopeGroupId(), folderName,
					"HSRA Supporting Documents");
			if(folderName.equals("Signature Radiation Declaration")||folderName.equals("Signature of Legal Person")
				||folderName.equals("Signature of Service Declaration")) { 
				  try {

<%@page import="com.nbp.mining.upload.document.commands.GettingDocumentData"%>
...
<%
int counterOfReqDoc =1;
int counterOfNonReqDoc =1;
...
if(Validator.isNotNull(request.getAttribute("miningLeaseApplicationId"))){
	applicationId= String.valueOf(request.getAttribute("miningLeaseApplicationId"));
	System.out.println("DOC APP ID miningApplicationId************** "+applicationId);
}
...
try{
	...
	System.out.println("DOC supportingDocumentsParentAndSubFolder************** "+supportingDocumentsParentAndSubFolder);
}catch(Exception e){
}
SimpleDateFormat dateFormatDoc = new SimpleDateFormat("yyyy-MM-dd");
/* if(sizeOfNonRequiredDocsList>0){ */
		...
		<div class="row">
			<div class="col-md-12">
				<div class="supporting-document-card-wrapper">
					<div class="left">
						<p class="supporting-document-card-upper-heading" id="requiredDoc<%=counterOfDocuments%>">
							<%=documentNames%> 
						</p>
					</div>
		...
		<div class="row">
			...
			<div class="uploaded-info" id="unpload-info-card<%=fileEntryId%>">
				<div class="left">
					...
					<p class="uploaded-info-size" id="document-size<%=counterOfDocuments%>"><%=fileSize %></p>
				</div>
				<div class="right">
...
<input type="text" hidden id="fileUrl<%=counterOfDocuments%>" value="<%=fileUrl%>">
<%counterOfDocuments++;	  %>
<%}
%>
<input type="hidden" id="requiredCounter"  name="requiredCounter" value="<%=counterOfReqDoc%>">
<% if(sizeOfNonRequiredDocsList>1){%>
<!-- <p class="supporting-document-subtitle">Optional Documents (Upload only those documents that are relevant to your application)</p> -->
<%
	...
	<div class="supporting-document-card mb-3">
		<div class="row">
			<div class="col-md-12">
				<div class="supporting-document-card-wrapper">
					<div class="left">
						<p class="supporting-document-card-upper-heading" id="notRequiredDoc<%=counterOfDocuments%>">
							<%=documentNamesNR%> 
						</p>
					</div>
					<div class="right text-nowrap">
		...
		<div class="row">
			...
			<%if(fileEntryNR!=null && fileEntryIdNR!=0){ %>
				...
				<div class="left">
					...
					<p class="uploaded-info-size" id="document-size"><%=fileSizeNR %></p>
				</div>
				<div class="right">
...
<input type="text" hidden id="fileUrl<%=counterOfDocuments%>" value="<%=fileUrlNR %>">
<% counterOfDocuments++;
counterOfNonReqDoc = counterOfDocuments;
}}
%>
<input type="hidden" id="requiredCounter"  name="requiredCounter" value="<%=sizeOfRequiredDocsList %>">
<input type="hidden" id="notRequiredCounter"  name="notRequiredCounter" value="<%=counterOfNonReqDoc%>">

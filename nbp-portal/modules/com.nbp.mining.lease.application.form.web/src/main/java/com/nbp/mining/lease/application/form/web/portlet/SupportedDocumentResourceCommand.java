...
public class SupportedDocumentResourceCommand implements MVCResourceCommand{
	...
	public boolean serveResource(ResourceRequest resourceRequest, ResourceResponse resourceResponse)
		...
		if(Validator.isNotNull(inputStream)){
				...
				FileEntry fileEntry =  _dlAppLocalService.addFileEntry(String.valueOf(CounterLocalServiceUtil.increment(DLFileEntry.class.getName())) +miningLeaseApplicationId, themeDisplay.getUserId(), themeDisplay.getScopeGroupId(),
	...
	private DLFolder createFolder(long userId,long miningLeaseApplicationId , long groupId) {
		DLFolder dlFolder = null;
			...
			try {
				...
				try {
				dlFolder = DLFolderLocalServiceUtil.getFolder(repositoryId, rootFolder.getFolderId(),String.valueOf(miningLeaseApplicationId));
				}catch (Exception e) {
					_log.error("No folder exist with name "+userId);
				}
				 if(Validator.isNull(dlFolder)){
					 dlFolder = DLFolderLocalServiceUtil.addFolder(String.valueOf(CounterLocalServiceUtil.increment()), userId, groupId, repositoryId, false, rootFolder.getFolderId(), String.valueOf(miningLeaseApplicationId), ROOT_FOLDER_DESCRIPTION, false, serviceContext);
				 }				

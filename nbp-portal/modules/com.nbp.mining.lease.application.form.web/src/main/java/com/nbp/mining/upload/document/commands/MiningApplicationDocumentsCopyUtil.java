...
public class MiningApplicationDocumentsCopyUtil {
	private static Log _log = LogFactoryUtil.getLog(MiningApplicationDocumentsCopyUtil.class.getName());
	...
	public static void copyFolder(long repositoryId, long sourceFolderId, long parentFolderId, String name,
		...
		try {
			destFolder = DLAppServiceUtil.addFolder(String.valueOf(CounterLocalServiceUtil.increment()), repositoryId,
					parentFolderId, String.valueOf(miningLeaseApplicationId), "Document Folder", serviceContext);
		} catch (PortalException e) {
			_log.error("Error*******" + e.getMessage());
		}

		try {
			copyFolder(sourceDlFolder, destFolder, serviceContext, userId, repositoryId, miningLeaseApplicationId);
		} catch (PortalException e) {
	...
	protected static void copyFolder(Folder srcFolder, Folder destFolder, ServiceContext serviceContext, long userId,
			long groupId, long miningLeaseApplicationId) throws PortalException {

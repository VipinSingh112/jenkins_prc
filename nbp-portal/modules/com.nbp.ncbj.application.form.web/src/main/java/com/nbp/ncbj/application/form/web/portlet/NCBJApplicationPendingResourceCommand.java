...
@Component(immediate = true, property = { "javax.portlet.name=" + NCBJApplicationFormPortletKeys.NCBJAPPLICATIONFORM,
		"mvc.command.name=/pending/ncbj/application" }, service = MVCResourceCommand.class)
public class NCBJApplicationPendingResourceCommand implements MVCResourceCommand {
	private static Log _log = LogFactoryUtil.getLog(NCBJApplicationPendingResourceCommand.class.getName());
	...
	public boolean serveResource(ResourceRequest resourceRequest, ResourceResponse resourceResponse)
		...
		String secondCategoryVal = ParamUtil.getString(resourceRequest, "secondCategoryVal");
		...
		if (Validator.isNull(ncbjApplication)) {
			ncbjApplication = NcbjApplicationLocalServiceUtil.createNcbjApplication(
					CounterLocalServiceUtil.increment(NCBJApplicationPendingResourceCommand.class.getName()));
		}
		...
		ncbjApplication.setIso22000CertificationValue(secondCategoryVal);

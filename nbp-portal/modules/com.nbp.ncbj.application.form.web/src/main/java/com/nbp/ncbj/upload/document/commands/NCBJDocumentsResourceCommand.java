package com.nbp.ncbj.upload.document.commands;
...
@Component(
		...
		property = {
			...
			"mvc.command.name=/generic/upload/in/ncbj"
		},
		service = MVCResourceCommand.class
	)
public class NCBJDocumentsResourceCommand implements MVCResourceCommand {
	private static long PARENT_FOLDER_ID = DLFolderConstants.DEFAULT_PARENT_FOLDER_ID;
	private static String ROOT_FOLDER_NAME = "NCBJ Supported Documents";
	private static Log _log = LogFactoryUtil.getLog(NCBJDocumentsResourceCommand.class);
@Override
	public boolean serveResource(ResourceRequest resourceRequest, ResourceResponse resourceResponse)
		...
		if (Validator.isNotNull(fileList) && fileList.length > 0) {
			dlFolder = com.nbp.ncbj.application.util.NCBjApplicationUtil.createSubFolder(

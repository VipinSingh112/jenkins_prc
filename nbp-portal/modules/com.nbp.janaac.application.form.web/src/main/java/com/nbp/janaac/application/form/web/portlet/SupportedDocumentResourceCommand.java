...
@Component(immediate = true, property = {
		...
		"mvc.command.name=/add/janaac/upload/document/url" }, service = MVCResourceCommand.class)
public class SupportedDocumentResourceCommand implements MVCResourceCommand {
	...
	public boolean serveResource(ResourceRequest resourceRequest, ResourceResponse resourceResponse)
		...
		try {
			...
			if (Validator.isNotNull(inputStream)) {
				...
				DLFolder dlFolder = createFolder(themeDisplay.getUserId(), janaacApplicationId,
						themeDisplay.getScopeGroupId());
				// commonsUtil.updateFolderPermission(dlFolder);
				commonsUtil.updateFolderPermissionGuest(dlFolder);
				commonsUtil.updateFolderPermissionGuest(dlFolder);
				FileEntry fileEntry = _dlAppLocalService.addFileEntry(fileName + janaacApplicationId,
				...
				ExpandoColumn docNoExpandoColumn = ExpandoColumnLocalServiceUtil.getColumn(expandoTable.getTableId(),
						JanaacApplicationFormPortletKeys.DOC_NO);
				if (docNoExpandoColumn == null) {
					docNoExpandoColumn = ExpandoColumnLocalServiceUtil.getColumn(themeDisplay.getCompanyId(),
							classNameId, expandoTable.getName(), JanaacApplicationFormPortletKeys.DOC_NO);
				}
				...
				resourceRequest.getPortletContext()
						.getRequestDispatcher("/supporting-documents/janaac-supporting-document-info.jsp")
	...
	private DLFolder createFolder(long userId, long healthApplicationId, long groupId) {
		...
		try {
			...
			if (Validator.isNull(dlFolder)) {
				dlFolder = DLFolderLocalServiceUtil.addFolder(String.valueOf(CounterLocalServiceUtil.increment()), userId, groupId, repositoryId, false,
						rootFolder.getFolderId(), String.valueOf(healthApplicationId), ROOT_FOLDER_DESCRIPTION, false,
						serviceContext);
			}

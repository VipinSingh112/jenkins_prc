...
String janaacAppId="";
String typeOFAccreditationService="";
...
if(Validator.isNotNull(request.getAttribute("typeOFAccreditationService"))){
	typeOFAccreditationService= String.valueOf(request.getAttribute("typeOFAccreditationService"));
}
if(Validator.isNotNull(request.getAttribute("janaacApplicationId"))){
	janaacAppId= String.valueOf(request.getAttribute("janaacApplicationId"));
}
...
try{
	 supportingDocumentsParentAndSubFolder =GettingUploadDoccumentData.getDocumentLibraryParentAndSubFolder(themeDisplay.getScopeGroupId(),
			Long.valueOf(janaacAppId)); 
}catch(Exception e){
}
...
String title = "";
String fileName = "";
String authorizedCopy = "hide";
String lowerDocumentName = "";
long fileEntryId = 0;
...
for(String documentNames:documentNameArray){
	...
	if(documentNames.equals("  Policies and objectives for fulfilling the requirements of the ISO/IEC 17025 standard")){
		authorizedCopy = "show";
	}else{
		authorizedCopy = "hide";
	}
	
	if(typeOFAccreditationService.equals("Accreditation of Certification Bodies to the ISO/IEC 17021-1 Standard")){
		if(documentNames.equals(" Documentation outlining organizational structure including relationship to other parts within the same legal entity")){
			lowerDocumentName = "If applicable, and related details for ALL personnel (both full time and contracted) involved in the certification process (i.e. job descriptions, resumes and/or other evidence of qualification competence) (CLAUSE 6.1.1, 7.2)";
		}
		if(documentNames.equals(" List of all current audit clients under the proposed accreditation scope indicating")){
			lowerDocumentName = "Client name, Date of last audit, Audit type (i.e. stage 1, stage 2, surveillance, recertification etc.), Date granted & date of expiry for current certificate (if applicable), Proposed/likely date of next audit, and geographical locations/company addresses within certified scope (indicating single vs. multisite operations)";
		}
	}
	
	if(typeOFAccreditationService.equals("Accreditation of Certification Bodies to the ISO/IEC 17065 Standard")){
		if(documentNames.equals(" List of all current audit clients under the proposed accreditation scope indicating: client name; date of the last audit")){
			lowerDocumentName = "Audit type (i.e. stage 1, stage 2, surveillance, recertification, etc.); date granted & date of expiry for the current certificate (if applicable); proposed/likely date of next audit, and geographical locations/company addresses within certified scope (indicating single vs. multisite operations)";
		}
		if(documentNames.equals(" Documentation outlining the organizational structure including relationship to other parts within the same legal entity")){
			lowerDocumentName = "If applicable, and related details for ALL personnel (both full time and contracted) involved in the certification process (i.e. job descriptions, resumes, and/or other evidence of qualification competence). See also Section B, Question 16 (CLAUSE 5.1)";
		}
	}
	if(typeOFAccreditationService.equals("Accreditation of Certification Bodies to the ISO/IEC 17024 Standard")){
		if(documentNames.equals(" Documentation outlining the organizational structure including relationship to other parts within the same legal entity")){
			lowerDocumentName = "If applicable, and related details for ALL personnel (both full time and contracted) involved in the certification process (i.e., job descriptions, resumes, and/or other evidence of qualification competence). See also Section B, Question 14 (CLAUSE 5.1.2)";
		}
	}
	try {
...
%>	
<p class="supporting-document-subtitle <%=authorizedCopy%>">
Authorized copies of management system documentation, including:
</p>
<div id="requiredDoc">
			...
			<div class="col-md-12">
				<div class="supporting-document-card-wrapper">
					<div class="left">
						...
						<p class="tab-form-sub-heading"><%=lowerDocumentName%></p>
					</div>
					<div class="right text-nowrap pl-3">
						<a id="documentUpload<%=counterOfDocuments%>" class="custom-upload <%=isDocumentUploaded%>" onchange="attachDetailed(this.id)" href="javascript:;" onclick="openDocumentUploadPopup('<%=counterOfDocuments%>','<%=documentNames%>')"> 
							<span class="mr-2">
		...
		<div class="row">
			...
			<div class="uploaded-info" id="unpload-info-card<%=fileEntryId%>">
				...
				<div class="right">
					...
					<a class="uploaded-delete" href="javascript:;"  onclick="opendelDocPopup('<%=fileEntryId%>','<%=counterOfDocuments%>')">
			...
			<%}%>
		<div class="modal fade commonsavepopup" id="janaacDeletepopup<%=fileEntryId%>" data-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true" style="display:none;">
    <div class="modal-dialog modal-dialog-centered my-0 modal-dialog-sm">
        <div class="modal-content">
            ...
            <div class="modal-footer border-0 d-flex justify-content-end">
                <a href="javascript:;" class="common-yellow-bg text-dark common-nav-links text-white popup-dashboard" onClick="janaacDeleteDocumentForGeneric('<%=fileEntryId%>');">
                    Yes<span class="ml-2">
                    </span>
                </a>
...
<div class="notRequiredDoc hide" id="notSupportingDoc">
<%
if(typeOFAccreditationService.equals("Accreditation of Medical Labs to the ISO 15189 Standard")){%>
	<p class="supporting-document-subtitle ">Authorized copy of management system documentation (including organizational and
			management structure and job descriptions for all persons with signatory status in the organizational structure)i.e.:
			</p>
	
	<%}else if(typeOFAccreditationService.equals("Accreditation of Inspection Bodies to the ISO/IEC 17020 Standard")){%>
	<p class="supporting-document-subtitle ">An authorized copy of the management system manual (including organizational 
	structure and documentation (e.g. job descriptions) for all positions in the organizational structure). (CLAUSE 8.2.1, 5.2.7); i.e.:
			</p>
<%}
%>
<!-- <p class="supporting-document-subtitle ">Authorized copy of management system documentation (including organizational and
		...
		<div class="row">
			<div class="col-md-12">
				<div class="supporting-document-card-wrapper">
					...
					<div class="right text-nowrap pl-3">
						<a id="documentUpload<%=counterOfDocuments%>" class="custom-upload <%=isDocumentUploadedNR%>" onchange="attachDetailed(this.id)" href="javascript:;" onclick="openDocumentUploadPopup('<%=counterOfDocuments%>','<%=documentNamesNR%>')"> 
							<span class="mr-2">
								<svg width="14" height="17" viewBox="0 0 14 17" fill="none" xmlns="http://www.w3.org/2000/svg">
		                       		<path d="M5 13H9C9.55 13 10 12.55 10 12V6.99997H11.59C12.48 6.99997 12.93 5.91997 12.3 5.28997L7.71 0.699971C7.32 0.309971 6.69 0.309971 6.3 0.699971L1.71 5.28997C1.08 5.91997 1.52 6.99997 2.41 6.99997H4V12C4 12.55 4.45 13 5 13ZM1 15H13C13.55 15 14 15.45 14 16C14 16.55 13.55 17 13 17H1C0.45 17 0 16.55 0 16C0 15.45 0.45 15 1 15Z" fill="#047247" />
		                       </svg> 
		...
		<div class="row">
			...
			<%if(fileEntryNR!=null && fileEntryIdNR!=0){ %>
				...
				<div class="right">
					...
					<a class="uploaded-delete" href="javascript:;"  onclick="opendelDocPopup('<%=fileEntryIdNR%>','<%=counterOfDocuments%>')">
						<svg width="14" height="18" viewBox="0 0 14 18" fill="none"
							xmlns="http://www.w3.org/2000/svg"> <path d="M1 18H13V4H1V18ZM3.46 8.88L4.87 7.47L7 9.59L9.12 7.47L10.53 8.88L8.41 11L10.53 13.12L9.12 14.53L7 12.41L4.88 14.53L3.47 13.12L5.59 11L3.46 8.88ZM10.5 1L9.5 0H4.5L3.5 1H0V3H14V1H10.5Z" fill="#282524" />
			            </svg>
					</a>

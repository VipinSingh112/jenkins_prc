...
public class FactoriesPrescribedFoodsPdf {
 ...
 return previewUrl;
    ...
    public static String replaceSignatureForOperatorFormA(long factoriesApplicationId, String folderName, long groupId,
    		long supportingDocumentsFolderId, ThemeDisplay themeDisplay) throws PortalException {
    	DLFolder signatureFolder = DLFolderLocalServiceUtil.getFolder(groupId, supportingDocumentsFolderId,
    			"Processed Food Act Signature");
    	List<DLFileEntry> supportinfDocFileEntries = DLFileEntryLocalServiceUtil.getFileEntries(groupId,
    			signatureFolder.getFolderId());
    	String previewUrl = "";
    	for (DLFileEntry dlFileEntry : supportinfDocFileEntries) {
	...
	public static String getPrescribedFoodFactRegstPdf(long factoriesApplicationId, ThemeDisplay themeDisplay,
		...
		//signatures
		 ...
		 try { 
			 signatureForOperatorFormA =replaceSignatureForOperatorFormA(processedFoodAct.getFactoriesApplicationId(), "Processed Food Act Signature", themeDisplay.getScopeGroupId(), 
					 supportingDocumentsFolder.getFolderId(),  themeDisplay); 
		 } catch (Exception e) {}
		...
		//pdf go in launch case
				if (checkPdfDownloadWithoutDocument.equals("false")) {
					pdfTemplate = getTemplateFromJournalArticlePDF(themeDisplay.getScopeGroupId(),
							"factories-prescribed-food-1");
				} else {
					try {
					pdfTemplate = getTemplateFromJournalArticlePDF
							(themeDisplay.getScopeGroupId(),
							"factories-prescribed-food");
					}catch (Exception e) {
		...
		try {
			...
			//Applicant details
				...
				"[$SignatureSiA$]","[$ProcessedDate$]","[$PaymentMethodNu$]"

...
public class OgtBankruptcyClaimToPropertyPdf {
	...
	public static String ogtClaimByCreditorPdf(long ogtApplicationId, ThemeDisplay themeDisplay,
		...
		if (checkPdfDownloadWithoutDocument.equals("false")) {
			pdfTemplate = getTemplateFromJournalArticlePDF(themeDisplay.getScopeGroupId(), "ogt-property-1");
		} else {
			try {
				pdfTemplate = getTemplateFromJournalArticlePDF(themeDisplay.getScopeGroupId(), "ogt-property");
			} catch (Exception e) {
		...
		try {
			pdfTemplate = StringUtil.replace(pdfTemplate, new String[] {
					...
					// CLAIM BY SPOUSE (Section 99) (Bankruptcy Only) 5
//					Validator.isNotNull(formC) && Validator.isNotNull(formC.getSpouseDate())
//							? dateFormat.format(formC.getSpouseDate())
//							: "",
//					Validator.isNotNull(formC) && Validator.isNotNull(formC.getClaimSpouseDesc())
//							? formC.getClaimSpouseDesc()
//							: "",

...
public class OgtProposalOrReceivershipPdf {
	...
	public static String ogtProposalOrReceivershipPdf(long ogtApplicationId, ThemeDisplay themeDisplay,
		...
		if (checkPdfDownloadWithoutDocument.equals("false")) {
			pdfTemplate = getTemplateFromJournalArticlePDF(themeDisplay.getScopeGroupId(), "ogt-proposal-1");
		} else {
			try {
				pdfTemplate = getTemplateFromJournalArticlePDF(themeDisplay.getScopeGroupId(), "ogt-proposal");
			} catch (Exception e) {
...
//claimant details
					...
					"[$Fno$]", "[$AppDate$]", "[$Tr$]",
//insolvent Person Detail
					"[$NatureDeb$]", "[$NameF$]", "[$NameL$]",
//section A				
					"[$Namecompy$]",

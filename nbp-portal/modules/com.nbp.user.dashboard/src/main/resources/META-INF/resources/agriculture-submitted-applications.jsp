...
<%@page import="com.nbp.agriculture.stages.services.service.AgriculturePermitLocalServiceUtil"%>
<%@page import="com.nbp.agriculture.stages.services.model.AgriculturePermit"%>
...
<%String checkDateInAgri="";
List<AgricultureApplicationStages> agriCurrentInProgressStages  =null;
List<AgricultureApplication> agriSubmitApp = new ArrayList<AgricultureApplication>();

if(showPIRApplications || superAdmin){
	agriApplicationData = AgricultureApplicationLocalServiceUtil.getAgricultureByStatus(UserDashboardPortletKeys.SUBMITTED);
	}else{
	try{
		agriApplicationData = AgricultureApplicationLocalServiceUtil.getAgricultureApplicationByS_U(themeDisplay.getUserId(), UserDashboardPortletKeys.SUBMITTED); 
	}catch(Exception e){
		e.printStackTrace();
	}
	}
	AgriculturePermit fullRegis=null;
	boolean expiredflag=false;
	for(AgricultureApplication appData:agriApplicationData){
		try{
		fullRegis=	AgriculturePermitLocalServiceUtil.getAgricultureBy_CI(appData.getCaseId());
		...
		if(!expiredflag){
			agriSubmitApp.add(appData);
		}
		expiredflag = false;
	}
for(AgricultureApplication applications :agriSubmitApp){ 
		 String notificationInAgri="hide";
		 String agriDefaultActive="current-active";
		 String agriStageStatus="Submitted";
		 String agricultureDashboardRenderURL = PortalUtil.getPortalURL(request)+"/group/guest"+"/agriculture-dashboard?agricultureApplicationId=";
		 int expirationAgriDocCount=DashboardUtil.getExpirationAgricultureDocCount(applications.getAgricultureApplicationId(), themeDisplay);
		 if(Validator.isNotNull(selectedApplication)){
			 agricultureDashboardRenderURL = agricultureDashboardRenderURL+String.valueOf(applications.getAgricultureApplicationId())+"&selectedApplication="+selectedApplication;
			}else{
				agricultureDashboardRenderURL = agricultureDashboardRenderURL+String.valueOf(applications.getAgricultureApplicationId());
			}
		try{
			agriCurrentInProgressStages = AgricultureApplicationStagesLocalServiceUtil.getAgricultureApplicationByCaseId(applications.getCaseId());
		}catch(Exception e){e.printStackTrace();}
		if(Validator.isNotNull(agriCurrentInProgressStages))agriDefaultActive="unvisited";
		  String pdfFileUrl ="";
		  ...
		  try{
			  checkDateInAgri=DashboardUtil.showAgriultureExpiredPermitNotification(applications.getCaseId());
			  if(Validator.isNotNull(checkDateInAgri)){
				  notificationInAgri="show";  
			  }
		  }catch(Exception e){}
	%>
				...
				<div class="progress-plus-wizard-card row cursor-pointer" onclick="getAgricultureSingleApplicationOverview('<%=agricultureDashboardRenderURL %>')">
					...
					<div class="tier-plus-issuedate-box col-lg-6 justify-content-between">
						<div class="single-box">
							...
							<p class="lower-title">
								<span class="one"> <%=applications.getPrimaryProduct() %></span>
							</p>
						</div>
					</div>
					...
					<form action="" method="post" role="form">
						<div class="form-wizard-header">
							<ul class="list-unstyled form-wizard-steps clearfix">
									<li class="start-active <%=agriDefaultActive%> farm-progress">
									<div class="dashboard-progress-card">
										...
										<div class="dashboard-progress-card-status"><span></span><%=agriStageStatus %></div>
									</div>
								</li>
							<%
							try{
								String stageJson = AgricultureApplicationStagesLocalServiceUtil.getAgricultureApplicationCurrentStages(applications.getCaseId());
		...
		<div class="row p-3">
			<div class="col-md-12">
				<div class="lower  dash-lower-multiple-links-box">
					<a class="dash-yellow-repeated-link"onclick="getSingleAgricultureApplicationSpecificStage('<%=agricultureDashboardRenderURL%>', 'overview', 'guest-overview-tab')"> Overview </a> 
				...
				<div class="col-md-11">
					<div class="approval-wrapper">
						<div class="approval-box-left">
							...
							<p class="three">Your <%=expirationAgriDocCount%> documents have expired.<span> Update Required </span></p>
						</div>
						<div class="approval-box-right">
							<a href="javascript:void(0);" class="common-green-bg btn" onclick="getSingleAgricultureApplicationSpecificStage('<%=agricultureDashboardRenderURL%>', 'agriculture-document', 'agriculture-document-tab')">Update document
								<span class="ml-2"> 
	...
	<div class="dash-light-cream-box common-dash-colored-boxes <%=notificationInAgri%>">
			<div class="row">
				<div class="col-md-1">
					<div class="icon">
						<span> 
				...
				<div class="col-md-11">
					<div class="approval-wrapper">
						<div class="approval-box-left">
							...
							<p class="three"><%=Validator.isNotNull(checkDateInAgri)?checkDateInAgri:"" %> <span> Renewal Required </span></p>
						</div>
						<div class="approval-box-right">
							<a href="javascript:void(0);" class="common-green-bg btn" onclick="getSingleAgricultureApplicationSpecificStage('<%=agricultureDashboardRenderURL%>', 'agriculture-document', 'agriculture-document-tab')">

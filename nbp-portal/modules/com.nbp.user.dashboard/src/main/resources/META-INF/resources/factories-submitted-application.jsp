...
<%@page import="com.nbp.factories.registration.stage.services.model.FactoriesApplicationStage"%>
<%@page import="com.nbp.factories.registration.stage.services.service.FactoriesApplicationStageLocalServiceUtil"%>
...
<%String checkDateInfact="";
    List<FactoriesApplicationStage> factoriesCurrentInProgressStage = null; 
    List<FactoriesApplicationRegistration> factoriesSubmitApp = new ArrayList<FactoriesApplicationRegistration>();
    
    if(superAdmin){
    	
    	factoriesApplicationData=FactoriesApplicationRegistrationLocalServiceUtil.getFactoriesRegstByStatus(UserDashboardPortletKeys.SUBMITTED);
    }else{
    	try{
    ...
    FactoriesCerticateOfRegis factoriesCerticateOfRegis=null;
    boolean expiredflag=false;
	for(FactoriesApplicationRegistration appData:factoriesApplicationData){
		try{
			factoriesCerticateOfRegis=	FactoriesCerticateOfRegisLocalServiceUtil.getFactoryCertifiResgiBy_CI(appData.getCaseId());
			if (factoriesCerticateOfRegis != null && factoriesCerticateOfRegis.getDateOfExpiration() != null) {
	            if (factoriesCerticateOfRegis.getDateOfExpiration().compareTo(new Date()) <= 0) {
			expiredflag=true;
	}}}catch(Exception e){
	}
		if(!expiredflag){
			factoriesSubmitApp.add(appData);
		}
		expiredflag =false;
	}
    
	for(FactoriesApplicationRegistration applications : factoriesSubmitApp){
		 String factoriesRenderURL = PortalUtil.getPortalURL(request)+"/group/guest"+"/factories-registration-dashboard?factoriesApplicationId=";
			if(Validator.isNotNull(selectedApplication)){
				factoriesRenderURL = factoriesRenderURL+String.valueOf(applications.getFactoriesApplicationId())+"&selectedApplication="+selectedApplication;
			}else{
				factoriesRenderURL = factoriesRenderURL+String.valueOf(applications.getFactoriesApplicationId());
			}
			String factoriesDefaultActive="current-active";
			String FactoriesStageStatus = "Submitted";
			 try{
				 factoriesCurrentInProgressStage =FactoriesApplicationStageLocalServiceUtil.getFactories_By_CaseId(applications.getCaseId());
					 }catch(Exception e){
						}
			 if (Validator.isNotNull(factoriesCurrentInProgressStage)) {
				 factoriesDefaultActive = "unvisited";
				} 
			String pdfNcraFPFileUrl ="";
			FileEntry pdfNcraFPFileEntry = null;
			try{
				  pdfNcraFPFileEntry = DLAppLocalServiceUtil.getFileEntry(applications.getPdfFileEntryId());
			pdfNcraFPFileUrl = DLURLHelperUtil.getDownloadURL(pdfNcraFPFileEntry, pdfNcraFPFileEntry.getFileVersion(), themeDisplay,
						StringPool.BLANK, false, true);
			}catch(Exception e){}
			int expirationCount=0;
			try{
				expirationCount=	DashboardUtil.getExpirationDocCount(applications.getFactoriesApplicationId(),themeDisplay,"Factories Supported Documents");
			}catch(Exception e){}
			String notification ="hide";
			 try{
				 checkDateInfact=DashboardUtil.showFactoriesExpiredLiceseNotification(applications.getCaseId());
			  if(Validator.isNotNull(checkDateInfact)){
				  notification="show";  
			  }
		  }catch(Exception e){
			  e.printStackTrace(); } 
...
%>
	...
	<div class="second-card dash-common-card">
		<div class="row p-3">
			<div class="col-md-12">
				<div class="progress-plus-wizard-card row cursor-pointer" onclick="getFactoriesSingleApplicationOverview('<%=factoriesRenderURL%>')">
					...
					<div class="tier-plus-issuedate-box col-lg-6 justify-content-between">
						<div class="single-box">
							...
							<p class="lower-title"><span class="one"> <%=applications.getTypeOfTransaction() %></span> </p>
						</div>
					...
					<div class="right-part d-flex col-lg-3 justify-content-end">
						<div class="dash-status-box">
						 ...
						 <div class="dash-action-box d-flex">
							<a href="<%=Validator.isNotNull(pdfNcraFPFileUrl)?pdfNcraFPFileUrl:"javascript:void(0);"%>" class="dash-action-btns mr-2 common-yellow-bg" data-toggle="tooltip" title="Download Application Transcript">
					...
					<form action="" method="post" role="form">
						<div class="form-wizard-header">
							<ul class="list-unstyled form-wizard-steps clearfix">
								<li class="start-active <%=factoriesDefaultActive%> farm-progress">
									<div class="dashboard-progress-card">
										...
										<div class="dashboard-progress-card-status"><span></span><%=FactoriesStageStatus %></div>
									</div>
								</li>
							<%try{
								String stageJson = FactoriesApplicationStageLocalServiceUtil.getfactoriesCurrentStage(applications.getCaseId());
           ...
           <div class="row p-3">
				...
				<div class="col-md-12">
					<div class="lower  dash-lower-multiple-links-box">
						<a class="dash-yellow-repeated-link" onclick="getSingleFactoriesApplicationSpecificStage('<%=factoriesRenderURL%>', 'overview', 'ncra-overview-tab') "> Overview </a> 
				...
				<div class="col-md-11">
					<div class="approval-wrapper">
						...
						<div class="approval-box-right">
							<a href="javascript:void(0);" class="common-green-bg btn" onclick="getSingleFactoriesApplicationSpecificStage('<%=factoriesRenderURL%>', 'document', 'document-tab')">Update document
								<span class="ml-2"> 
				...
				<div class="col-md-11">
					<div class="approval-wrapper">
						<div class="approval-box-left">
							...
							<p class="three"><%=Validator.isNotNull(checkDateInfact)?checkDateInfact:"" %> <span> Renewal Required </span></p>
						</div>
						<%-- <div class="approval-box-right">

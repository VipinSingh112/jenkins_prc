...
<%@page import="com.nbp.medical.stages.service.model.MedicalApplicationStages"%>
<%@page import="com.liferay.portal.kernel.json.JSONObject"%>
<%@page import="com.liferay.portal.kernel.json.JSONFactoryUtil"%>
<%@page import="com.liferay.portal.kernel.json.JSONArray"%>
<%@page import="com.nbp.medical.stages.service.service.MedicalApplicationStagesLocalServiceUtil"%>
...
<%
List<MedicalFacilitiesApp> medicalSubmitApp = new ArrayList<MedicalFacilitiesApp>();

if(superAdmin){
	medicalApplicationData=MedicalFacilitiesAppLocalServiceUtil.getMedicalFacilitiesAppBy_Status(UserDashboardPortletKeys.SUBMITTED);
}else{
	try{
...
MedicalCertificateIssuance medicalCertificateIssuance = null;
boolean expiredflag =false;
for(MedicalFacilitiesApp appData:medicalApplicationData){
	try{
		medicalCertificateIssuance=MedicalCertificateIssuanceLocalServiceUtil.getCertificateIssuance(appData.getCaseId());
		if (medicalCertificateIssuance != null && medicalCertificateIssuance.getDateOfLicenseExpiration() != null) {
            if (medicalCertificateIssuance.getDateOfLicenseExpiration().compareTo(new Date()) <= 0) {
			expiredflag=true;
		}}}catch(Exception e){}
	if(!expiredflag){
		medicalSubmitApp.add(appData);
	}
	expiredflag =false;
}
	for(MedicalFacilitiesApp applications : medicalSubmitApp){ 
		 String medicalRenderURL = PortalUtil.getPortalURL(request)+"/group/guest"+"/institutional-healthcare-facilities-dashboard?medicalApplicationId=";
			if(Validator.isNotNull(selectedApplication)){
				medicalRenderURL = medicalRenderURL+String.valueOf(applications.getMedicalFacilitiesAppId())+"&selectedApplication="+selectedApplication;
			}else{
				medicalRenderURL = medicalRenderURL+String.valueOf(applications.getMedicalFacilitiesAppId());
			}
			String medicalDefaultActive="current-active";
			String medicalStageStatus="Submitted";

			List<MedicalApplicationStages> currentInProgressStage =null;

			try{
				 currentInProgressStage = MedicalApplicationStagesLocalServiceUtil.getMedicalByCI(applications.getCaseId());
				
			}catch(Exception e){	
			}
			if(Validator.isNotNull(currentInProgressStage)){
				medicalDefaultActive="unvisited";
			}
			String pdfMedicalFileUrl ="";
			FileEntry pdfMedicalFileEntry = null;
			try{
				  pdfMedicalFileEntry = DLAppLocalServiceUtil.getFileEntry(applications.getPdfFileEntryId());
			pdfMedicalFileUrl = DLURLHelperUtil.getDownloadURL(pdfMedicalFileEntry, pdfMedicalFileEntry.getFileVersion(), themeDisplay,
						StringPool.BLANK, false, true);
			}catch(Exception e){}
			int expirationCount=0;
			try{
				expirationCount=	DashboardUtil.getExpirationDocCount(applications.getMedicalFacilitiesAppId(),themeDisplay,"Medical Supported Documents");
			}catch(Exception e){}
			String checkDate="";
			String notification="hide";
			try{
				checkDate=DashboardUtil.showMedicalExpiredLiceseNotification(applications.getCaseId());
...
%>
	...
	<div class="second-card dash-common-card">
		<div class="row p-3">
			<div class="col-md-12">
				<div class="progress-plus-wizard-card row cursor-pointer" onclick="getMedicalSingleApplicationOverview('<%=medicalRenderURL%>')">
					...
					<div class="right-part d-flex col-lg-3 justify-content-end">
						<div class="dash-status-box">
						 ...
						 <div class="dash-action-box d-flex">
							<a href="<%=Validator.isNotNull(pdfMedicalFileUrl)?pdfMedicalFileUrl:"javascript:void(0);"%>" class="dash-action-btns mr-2 common-yellow-bg" data-toggle="tooltip" title="Download Application Transcript">
					...
					<form action="" method="post" role="form">
						<div class="form-wizard-header">
							<ul class="list-unstyled form-wizard-steps clearfix">
								<li class="start-active <%=medicalDefaultActive%> farm-progress">
									<div class="dashboard-progress-card">
										...
										<div class="dashboard-progress-card-status">
											<span></span><%=medicalStageStatus %>
										</div>
									</div>
								</li>
							<%
							String stageJson = MedicalApplicationStagesLocalServiceUtil.getNcbjApplicaitonCurrentStages(applications.getCaseId());
		...
		<div class="row p-3">
				<div class="col-md-12">
					<div class="lower  dash-lower-multiple-links-box">
						<a class="dash-yellow-repeated-link" onclick="getSingleMedicalFacilitiesApplicationSpecificStage('<%=medicalRenderURL%>', 'overview', 'medical-facilities-overview-tab') "> Overview </a> 
				...
				<div class="col-md-11">
					<div class="approval-wrapper">
						...
						<div class="approval-box-right">
							<a href="javascript:void(0);" class="common-green-bg btn" onclick="getSingleMedicalApplicationSpecificStage('<%=medicalRenderURL%>', 'medical-document', 'medical-document-tab')">Update document
								<span class="ml-2"> 

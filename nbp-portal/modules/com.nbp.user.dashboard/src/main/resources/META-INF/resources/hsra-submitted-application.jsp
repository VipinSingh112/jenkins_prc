...
<%@page import="com.nbp.hsra.stage.services.service.HSRAApplicationLicenseIssuanceLocalServiceUtil"%>
<%@page import="com.nbp.hsra.stage.services.model.HSRAApplicationLicenseIssuance"%>
<%@page import="com.nbp.hsra.application.service.service.HsraApplicationLocalServiceUtil"%>
<%@page import="com.nbp.hsra.stage.services.service.HsraApplicationStagesLocalServiceUtil"%>
<%@page import="com.nbp.hsra.stage.services.model.HsraApplicationStages"%>
...
<%
  List<HsraApplicationStages> hsraCurrentInProgressStages=null;


List<HsraApplication> hsraSubmitApp = new ArrayList<HsraApplication>();
if (superAdmin) {
	hsraApplicationData = HsraApplicationLocalServiceUtil.getHsraByStatus(UserDashboardPortletKeys.SUBMITTED);	
}else{
	try{
		hsraApplicationData = HsraApplicationLocalServiceUtil.getHsraByS_U(themeDisplay.getUserId(),UserDashboardPortletKeys.SUBMITTED);
	}catch(Exception e){
	}
	}
	
	HSRAApplicationLicenseIssuance fullRegis=null;
		boolean expiredflag=false;
		for(HsraApplication appData:hsraApplicationData){
			try{
			fullRegis=	HSRAApplicationLicenseIssuanceLocalServiceUtil.getHSRAApplicationLicenseIssuanceByCI(appData.getCaseId());
			if (fullRegis != null && fullRegis.getDateOfExpiry() != null) {
	            if (fullRegis.getDateOfExpiry().compareTo(new Date()) <= 0) {
			
				expiredflag=true;
		}}}catch(Exception e){
		}
			if(!expiredflag){
				hsraSubmitApp.add(appData);
			}
			expiredflag = false;
		}

	for(HsraApplication applications : hsraSubmitApp){ 
		 String hsraRenderUrl = PortalUtil.getPortalURL(request)+"/group/guest"+"/hsra-dashboard?hsraApplicationId=";
			if(Validator.isNotNull(selectedApplication)){
				hsraRenderUrl = hsraRenderUrl+String.valueOf(applications.getHsraApplicationId())+"&selectedApplication="+selectedApplication;
			}else{
				hsraRenderUrl = hsraRenderUrl+String.valueOf(applications.getHsraApplicationId());
			}
		 	String hsraDefaultActive = "current-active";
			String hsraStageStatus = "Submitted";
			 try{
				 hsraCurrentInProgressStages = HsraApplicationStagesLocalServiceUtil.getHsraByCI(applications.getCaseId());
					 }catch(Exception e){
						}
			 if (Validator.isNotNull(hsraCurrentInProgressStages)) {
				 hsraDefaultActive = "unvisited";
				} 
				int expirationCount=0;
				try{
					expirationCount=	DashboardUtil.getExpirationDocCount(applications.getHsraApplicationId(),themeDisplay,"HSRA Supporting Documents");
				}catch(Exception e){}
				
			  String pdfHsraFileUrl ="";
			  FileEntry pdfHsraFileEntry = null;
			  try{
				  pdfHsraFileEntry = DLAppLocalServiceUtil.getFileEntry(applications.getPdfFileEntryId());
				  pdfHsraFileUrl = DLURLHelperUtil.getDownloadURL(pdfHsraFileEntry, pdfHsraFileEntry.getFileVersion(), themeDisplay,
						StringPool.BLANK, false, true);
			  }catch(Exception e){} 
			  String checkDate="";
			  String notification="hide";
			  try{
				  checkDate=DashboardUtil.showHsraExpiredLiceseNotification(applications.getCaseId());
...
%>
	...
	<div class="second-card dash-common-card">
		<div class="row p-3">
			<div class="col-md-12">
				<div class="progress-plus-wizard-card row cursor-pointer" onclick="getHSRASingleApplicationOverview('<%=hsraRenderUrl%>')">
					...
					<div class="right-part d-flex col-lg-3 justify-content-end">
						<div class="dash-status-box">
						 ...
						 <div class="dash-action-box d-flex">
							<a href="<%=Validator.isNotNull(pdfHsraFileUrl)?pdfHsraFileUrl:"javascript:void(0)"%>" class="dash-action-btns mr-2 common-yellow-bg" data-toggle="tooltip" title="Download Application Transcript">
					...
					<div class="form-wizard">
						<form action="" method="post" role="form">
							...
							<ul class="list-unstyled form-wizard-steps clearfix">
								<li class="start-active <%=hsraDefaultActive%> farm-progress">
									<div class="dashboard-progress-card">
										...
										<div class="dashboard-progress-card-status"><span></span><%=hsraStageStatus %></div>
									</div>
								</li>
							<%try{
									String stageJson = HsraApplicationStagesLocalServiceUtil.getHsraApplicaitonCurrentStages(applications.getCaseId());
		...
		<div class="row p-3">
				<div class="col-md-12">
					<div class="lower  dash-lower-multiple-links-box">
						<a class="dash-yellow-repeated-link" onclick="getHSRASingleApplicationSpecificStage('<%=hsraRenderUrl%>', 'overview', 'hsra-overview-tab') "> Overview </a> 
				...
				<div class="col-md-11">
					<div class="approval-wrapper">
						...
						<div class="approval-box-right">
							<a href="javascript:void(0);" class="common-green-bg btn" onclick="getHsraApplicationSpecificStage('<%=hsraRenderUrl%>', 'hsra-document', 'hsra-document-tab')">Update document
								<span class="ml-2"> 

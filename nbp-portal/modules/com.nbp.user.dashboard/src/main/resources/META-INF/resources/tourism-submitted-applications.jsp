...
<%String checkDateInTourism="";
List<TourismApplicationStages> tourismCurrentInProgressStages  =null;

List<TourismApplication> tourismSubmitApp = new ArrayList<TourismApplication>();

if(showTourismApplications || superAdmin) {
	tourismApplicationData = TourismApplicationLocalServiceUtil.getTourismByStatus(UserDashboardPortletKeys.SUBMITTED); 
}else{
	try{
	...
	TourismPermit fullRegis=null;
		boolean expiredflag=false;
		for(TourismApplication appData:tourismApplicationData){
			try{
			fullRegis=	TourismPermitLocalServiceUtil.getTourisPermitBy_CI(appData.getCaseId());
			...
			if(!expiredflag){
				tourismSubmitApp.add(appData);
			}
			expiredflag = false;
		}
 for(TourismApplication applications :tourismSubmitApp){ 
	     String notificationInTourism="hide";
		 String tourismDefaultActive="current-active";
		 String tourismStageStatus="Submitted";
		 
		 String tourismDashboardRenderURL = PortalUtil.getPortalURL(request) + "/group/guest"
					+ "/tourism-dashboard?tourismApplicationId=";
		 int expirationTourismDocumentCount=DashboardUtil.getExpirationTourismDocCount(applications.getTourismApplicationId(), themeDisplay);
		 if(Validator.isNotNull(selectedApplication)){
			 tourismDashboardRenderURL = tourismDashboardRenderURL+ String.valueOf(applications.getTourismApplicationId())+"&selectedApplication="+selectedApplication;
		}else{
			tourismDashboardRenderURL = tourismDashboardRenderURL+ String.valueOf(applications.getTourismApplicationId());
		}
		try{
			tourismCurrentInProgressStages = TourismApplicationStagesLocalServiceUtil.getTAS_CaseId(applications.getCaseId());
		}catch(Exception e){
			e.printStackTrace();
		}
		if(Validator.isNotNull(tourismCurrentInProgressStages)){
			tourismDefaultActive="unvisited";
		}
		  String pdfFileUrl ="";
		  ...
		  try{
			  checkDateInTourism=DashboardUtil.showTourismExpiredPermitNotification(applications.getCaseId());
			  if(Validator.isNotNull(checkDateInTourism)){
				  notificationInTourism="show";  
			  }
		  }catch(Exception e){}
	%>
	<div class="second-card dash-common-card">
				...
				<div class="progress-plus-wizard-card row cursor-pointer" onclick="getTourismSingleApplicationOverview('<%=tourismDashboardRenderURL%>')">
					...
					<div class="tier-plus-issuedate-box col-lg-6 justify-content-between">
						<div class="single-box">
							<p class="upper-title">License Category</p>
							<p class="lower-title"><span class="one"> <%=applications.getLicenseCategory()%></span></p>
						</div>
					</div>
					...
					<form action="" method="post" role="form">
						<div class="form-wizard-header">
							<ul class="list-unstyled form-wizard-steps clearfix">
									<li class="start-active <%=tourismDefaultActive%> film-progress">
									<div class="dashboard-progress-card">
										...
										<div class="dashboard-progress-card-status">
											<span></span><%=tourismStageStatus %>
										</div>
									</div>
								</li>
							<%try{
								String stageJson = TourismApplicationStagesLocalServiceUtil.getTourismCurrentStages(applications.getCaseId());
		...
		<div class="row p-3">
			<div class="col-md-12">
				<div class="lower  dash-lower-multiple-links-box">
					<a class="dash-yellow-repeated-link" onclick="getTourismSingleApplicationOverview('<%=tourismDashboardRenderURL%>')"> Overview </a> 
		...
		<%if (expirationTourismDocumentCount > 0) {%>
				...
				<div class="col-md-11">
					<div class="approval-wrapper">
						<div class="approval-box-left">
							...
							<p class="three">Your <%=expirationTourismDocumentCount%> documents have expired.<span> Update Required </span></p>
						</div>
						<div class="approval-box-right">
							<a href="javascript:void(0);" class="common-green-bg btn" onclick="getSingletourismApplicationSpecificStage('<%=tourismDashboardRenderURL%>', 'tourism-document', 'tourism-document-tab')">
								Update document
								<span class="ml-2"> 
	...
	<div class="dash-light-cream-box common-dash-colored-boxes <%=notificationInTourism%>">
			<div class="row">
				<div class="col-md-1">
					<div class="icon">
						<span> 
				...
				<div class="col-md-11">
					<div class="approval-wrapper">
						<div class="approval-box-left">
							...
							<p class="three"><%=Validator.isNotNull(checkDateInTourism)?checkDateInTourism:"" %> <span> Renewal Required </span></p>
						</div>
						<div class="approval-box-right">
							<a href="javascript:void(0);" class="common-green-bg btn" onclick="getSingletourismApplicationSpecificStage('<%=tourismDashboardRenderURL%>', 'tourism-document', 'tourism-document-tab')">

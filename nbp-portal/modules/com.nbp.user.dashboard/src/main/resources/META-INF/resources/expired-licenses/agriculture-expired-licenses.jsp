...
<%
	List<AgriculturePermit> fullRegistrations=null;
	AgricultureApplication agricultureAppli=null;
	
try{
		if(superAdmin){
			fullRegistrations=AgriculturePermitLocalServiceUtil.getAgriculturePermits(QueryUtil.ALL_POS, QueryUtil.ALL_POS);
		}else{
			try{
...
if(superAdmin&&Validator.isNotNull(fullRegistrations)){
	for(AgriculturePermit fullRegis:fullRegistrations){
		if (fullRegis.getDateOfExpiration() != null && 
	            fullRegis.getDateOfExpiration().compareTo(new Date()) <= 0) {
	try{
		agricultureAppli=AgricultureApplicationLocalServiceUtil.getAgricultureBy_CI(fullRegis.getCaseId());
	}catch(Exception e){
	}
	String agricultureDashboardRenderURL=null;
	try{
	
	 agricultureDashboardRenderURL = PortalUtil.getPortalURL(request)+"/group/guest"+"/agriculture-dashboard?agricultureApplicationId="+agricultureAppli.getAgricultureApplicationId()+"&_com_nbp_agriculture_dashboard_web_AgricultureDashboardWebPortlet__stageTab=agriculture-permit-tab"
			+"&_com_nbp_agriculture_dashboard_web_AgricultureDashboardWebPortlet_stageName=agriculture-permit";
	}catch(Exception e){
	}
	if(agricultureAppli!=null&&agricultureAppli.getApplicationNumber()!=null){
			%>
							...
							<div class="left-part">
								...
								<div class="dash-conditional-box">
									<div class="dash-conditional-box-left">
										...
										<%=Validator.isNotNull(agricultureAppli)&&Validator.isNotNull(agricultureAppli.getApplicationNumber())?agricultureAppli.getApplicationNumber():""%>
										</span>
									</div>
								</div>
							</div>
						...
						<div class="tier-plus-issuedate-box col-lg-6 justify-content-between">
							<div class="single-box">
								...
								<p class="lower-title">
									...
									<%=Validator.isNotNull(agricultureAppli)&&Validator.isNotNull(agricultureAppli.getRegistrationStatus())?agricultureAppli.getRegistrationStatus():""%>
									</span>
								</p>
							</div>
						</div>
		...
		<%}}}}else{
			for(AgricultureApplication fullRegis:agriApplicationData){
				AgriculturePermit agriculturePermitData =null;
			
			try{
				agriculturePermitData=AgriculturePermitLocalServiceUtil.getAgricultureBy_CI(fullRegis.getCaseId());
			}catch(Exception e){
				
			}
				if (agriculturePermitData!=null&& agriculturePermitData.getDateOfExpiration() != null && 
						agriculturePermitData.getDateOfExpiration().compareTo(new Date()) <= 0) {
			
					String agricultureDashboardRenderURL=null;
					try{
					
					 agricultureDashboardRenderURL = PortalUtil.getPortalURL(request)+"/group/guest"+"/agriculture-dashboard?agricultureApplicationId="+fullRegis.getAgricultureApplicationId()+"&_com_nbp_agriculture_dashboard_web_AgricultureDashboardWebPortlet__stageTab=agriculture-permit-tab"
							+"&_com_nbp_agriculture_dashboard_web_AgricultureDashboardWebPortlet_stageName=agriculture-permit";
					}catch(Exception e){
					}
					%>
								...
								<div class="tier-plus-issuedate-box col-lg-6 justify-content-between">
									<div class="single-box">
										...
										<p class="lower-title">
											...
											<%=fullRegis.getRegistrationStatus()%>
											</span>
										</p>
									</div>
								</div>

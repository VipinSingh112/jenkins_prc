...
<%
	List<HSRAApplicationLicenseIssuance> fullRegistrations=null;
	HsraApplication hsraAppli=null;
	try{
		
		if(superAdmin){
			fullRegistrations=HSRAApplicationLicenseIssuanceLocalServiceUtil.getHSRAApplicationLicenseIssuances(QueryUtil.ALL_POS, QueryUtil.ALL_POS);
		}else{
			try{
	...
	if(superAdmin&&Validator.isNotNull(fullRegistrations)){
		try{
	for(HSRAApplicationLicenseIssuance fullRegis:fullRegistrations){
		if (fullRegis.getDateOfExpiry() != null && 
	            fullRegis.getDateOfExpiry().compareTo(new Date()) <= 0) {
	try{
		hsraAppli=HsraApplicationLocalServiceUtil.getHsraByCaseId(fullRegis.getCaseId());
	}catch(Exception e){
	}
	if(hsraAppli!=null){
	String hsraRenderUrl =null;
	try{
		hsraRenderUrl = PortalUtil.getPortalURL(request)+"/group/guest"+"/hsra-dashboard?hsraApplicationId="
				+hsraAppli.getHsraApplicationId()+"&_com_nbp_hsra_dashboard_web_HsraDashboardPortlet__stageTab=hsra-certificate-tab"
						+"&_com_nbp_hsra_dashboard_web_HsraDashboardPortlet_stageName=hsra-certificate";
	}catch(Exception e){
		e.printStackTrace();
	}
	if(hsraAppli!=null&&hsraAppli.getApplicationNumber()!=null){
	%>
							...
							<div class="left-part">
								...
								<div class="dash-conditional-box">
									<div class="dash-conditional-box-left">
										...
										<%=Validator.isNotNull(hsraAppli)&&Validator.isNotNull(hsraAppli.getApplicationNumber())?hsraAppli.getApplicationNumber():""%>
										</span>
									</div>
								</div>
							</div>
						...
						<div class="tier-plus-issuedate-box col-lg-6 justify-content-between">
							<div class="single-box">
								...
								<p class="lower-title">
									...
									<%=Validator.isNotNull(hsraAppli)&&Validator.isNotNull(hsraAppli.getTypeOfApplicant())?hsraAppli.getTypeOfApplicant():""%>
									</span>
								</p>
							</div>
						</div>
	...
	for(HsraApplication fullRegis:hsraApplicationData){
		HSRAApplicationLicenseIssuance hsraLicenceData=null;
		try{
			hsraLicenceData=HSRAApplicationLicenseIssuanceLocalServiceUtil.getHSRAApplicationLicenseIssuanceByCI(fullRegis.getCaseId());
		}catch(Exception e){}
		if (hsraLicenceData!=null && hsraLicenceData.getDateOfExpiry() != null && 
				hsraLicenceData.getDateOfExpiry().compareTo(new Date()) <= 0) {
	
			String hsraRenderUrl =null;
			try{
				hsraRenderUrl = PortalUtil.getPortalURL(request)+"/group/guest"+"/hsra-dashboard?hsraApplicationId="
						+fullRegis.getHsraApplicationId()+"&_com_nbp_hsra_dashboard_web_HsraDashboardPortlet__stageTab=hsra-certificate-tab"
								+"&_com_nbp_hsra_dashboard_web_HsraDashboardPortlet_stageName=hsra-certificate";
			}catch(Exception e){
				e.printStackTrace();
			}

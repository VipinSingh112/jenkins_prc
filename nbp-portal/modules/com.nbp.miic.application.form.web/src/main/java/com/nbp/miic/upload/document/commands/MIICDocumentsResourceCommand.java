...
@Component(immediate = true, property = { "javax.portlet.name=" + MIICApplicationFormPortletKeys.MIICAPPLICATIONFORM,
		"mvc.command.name=/upload/documents/in/miic" }, service = MVCResourceCommand.class)
public class MIICDocumentsResourceCommand implements MVCResourceCommand {
	private static Log _log = LogFactoryUtil.getLog(MIICDocumentsResourceCommand.class);
	...
	if (folderName.equals("Generic Documents")) {
		...
		if (Validator.isNotNull(fileList) && fileList.length > 0) {
			dlFolder = com.nbp.miic.application.util.MIICApplicationUtil.createSubFolder(themeDisplay.getUserId(),
					miicApplicationId, themeDisplay.getScopeGroupId(), folderName, " Forms Documents");

...
@Component(immediate = true, property = {
		...
		"mvc.command.name=/upload/signature/in/sez"
		},
		service = MVCResourceCommand.class
		)
public class SezStatusDocumentsResourceCommand implements MVCResourceCommand {
	private static long PARENT_FOLDER_ID = DLFolderConstants.DEFAULT_PARENT_FOLDER_ID;
	private static String ROOT_FOLDER_NAME = "SEZ Status Supported Documents";
	private static Log _log = LogFactoryUtil.getLog(SezStatusDocumentsResourceCommand.class);
@Override
	public boolean serveResource(ResourceRequest resourceRequest, ResourceResponse resourceResponse)
		...
		if (Validator.isNotNull(fileList) && fileList.length > 0) {
			dlFolder = com.nbp.sez.status.application.util.SezStatusApplicationUtil.createSubFolder(
					themeDisplay.getUserId(), sezStatusApplicationId, themeDisplay.getScopeGroupId(), folderName,
					"SEZ Status Forms Documents");
			...
			for (File fileItem : fileList) {
				try {
					...
					if (Validator.isNotNull(fileItem) && Validator.isNotNull(fileName)) {
						...
						System.out.println("json array of generic - "+jsonArrayWithUrl);

...
try{
	if(applicationPayment.getPaymentMethod().equals("Bank Draft")){
...
try{
	selPaymentMethod = applicationPayment.getPaymentMethod();
}catch(Exception e){
}
%>
<fieldset class="wizard-fieldset" id="SezStatusFeePaymentFieldset"
	name="APPLICATION FEES PAYMENT">
		...
		<div class="row" id="rm-payment-method">
			<div class="col-md-12">
				<ul class="d-flex">
					<li class="arrowed" id="choose-payment-method"><span>Payment
							method</span></li>
					<li class="arrowed hide" id="payment-method"><span>Cheque</span></li>
				</ul>
			</div>
			<div class="col-md-7">
			  ...
			  <path
				...
				<path
									...
									<div class="right">
										<input type="file" id="sez-payment-upload"
											onchange="attachDetailed(this.id)" 
											name="<portlet:namespace/>sez-payment-upload"
											hidden=""> <label
											class="btn btn-primary ml-5 transparent-btn"
											for="sez-payment-upload">Upload</label>
									</div>
								</div>
								<div class="supporting-document-card">
								<span id="sez-payment-file-chosen" class="attach-detailed-overview"></span>
													<%
					if(sezStatusApplicationId>0){	
					boolean isFilePresent = false; 
				
				try{	
					DLFolder subFolderPayment=   GettingDocumentData.getDocumentLibraryParentAndSubFolder(themeDisplay.getScopeGroupId(), sezStatusApplicationId);
					supportingDocumentsFolderSubFolder = DLFolderLocalServiceUtil.getFolder(themeDisplay.getScopeGroupId(),
							subFolderPayment.getFolderId(), "Fee Reciept");
				...
				if(Validator.isNotNull(supportinfDocFileEntries)){
					for(DLFileEntry uploadedFile:supportinfDocFileEntries){
						try {
							...
							fileUrl=DLURLHelperUtil.getDownloadURL(fileEntryIdDocPay, fileEntryIdDocPay.getFileVersion(), themeDisplay,
			    ...
			    <% } %>
			     ...
			     <%} %>
								...
								<!-- <p class="tab-form-sub-heading hide fee-msg">Document has been uploaded successfully</p> -->
							</div>
						</div>
					</div>
					<div class="tab-plus-wizard">
							...
							<div class="col-md-6">
								<div class="form-group">
									...
									<input type="text" class="form-control" id="amountPaid"
										value="<%=Validator.isNotNull(applicationPayment)&&Validator.isNotNull(applicationPayment.getAmountPaid())?applicationPayment.getAmountPaid():"" %>" placeholder=""
									...
									<div class="select-wrapper position-relative">
										<select class="form-select form-control"
											...
											<option value="" disabled="" selected="">Please
												Select</option>
											<option value="JMD" <%=Validator.isNotNull(applicationPayment)&&Validator.isNotNull(applicationPayment.getAmountCurrency())&&applicationPayment.getAmountCurrency().equals("JMD")?"selected":"" %>>JMD</option>
											<option value="USD" <%=Validator.isNotNull(applicationPayment)&&Validator.isNotNull(applicationPayment.getAmountCurrency())&&applicationPayment.getAmountCurrency().equals("USD")?"selected":"" %> >USD</option>
											<option value="POUND" <%=Validator.isNotNull(applicationPayment)&&Validator.isNotNull(applicationPayment.getAmountCurrency())&&applicationPayment.getAmountCurrency().equals("POUND")?"selected":"" %> >POUND</option>
											<option value="EURO" <%=Validator.isNotNull(applicationPayment)&&Validator.isNotNull(applicationPayment.getAmountCurrency())&&applicationPayment.getAmountCurrency().equals("EURO")?"selected":"" %>  >EURO</option>
										</select>
									</div>
								</div>
							</div>
			...
			<div class="col-md-5 ">
				<div class="side-line p-4">
					...
					<div class="row">
						<div class="col-md-12">
							<p class="mb-0">SEZ Status</p>
							<div class="d-flex ml-2">
								<span>
									<svg width="17" height="35" viewBox="0 0 17 35" fill="none" xmlns="http://www.w3.org/2000/svg">
										<line x1="0.5" y1="2.18558e-08" x2="0.499999" y2="31" stroke="#047247"/>
										<path d="M16.3536 31.3536C16.5488 31.1583 16.5488 30.8417 16.3536 30.6464L13.1716 27.4645C12.9763 27.2692 12.6597 27.2692 12.4645 27.4645C12.2692 27.6597 12.2692 27.9763 12.4645 28.1716L15.2929 31L12.4645 33.8284C12.2692 34.0237 12.2692 34.3403 12.4645 34.5355C12.6597 34.7308 12.9763 34.7308 13.1716 34.5355L16.3536 31.3536ZM0 31.5H16V30.5H0V31.5Z" fill="#047247"/>
									</svg>
								</span>
				  ...
				  <h1 class="fee-heading">Security Bond</h1>
				  <p class="mb-0">US$ <span id="security-bond-fee"></span></p>
			  </div>
			  <div class="d-flex align-items-center justify-content-between">
				  <h1 class="fee-heading">License Fee</h1>
				  <p class="mb-0">US$ <span id="license-fee"></span></p>
			  </div> -->
							<div class="d-flex align-items-center justify-content-between">
								<h1 class="fee-heading">Application Fee</h1>
								<p class="mb-0 feeAmount">500</p>
							</div>
						</div>
					</div>
					<div class="row">

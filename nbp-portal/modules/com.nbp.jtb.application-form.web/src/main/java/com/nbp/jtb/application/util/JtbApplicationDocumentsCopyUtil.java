...
public class JtbApplicationDocumentsCopyUtil {
	private static Log _log = LogFactoryUtil.getLog(JtbApplicationDocumentsCopyUtil.class.getName());
	...
	public synchronized static void copyFolder(long repositoryId, long sourceFolderId, long parentFolderId, String name,
		...
		try {
			destFolder = DLAppServiceUtil.addFolder(String.valueOf(CounterLocalServiceUtil.increment()), repositoryId,
					parentFolderId, String.valueOf(jtbApplicationId), "Document Folder", serviceContext);
		} catch (PortalException e) {
			_log.error("Error*******" + e.getMessage());
		}

		try {
			copyFolder(sourceDlFolder, destFolder, serviceContext, userId, repositoryId, jtbApplicationId);
		} catch (PortalException e) {
	...
	protected synchronized static void copyFolder(Folder srcFolder, Folder destFolder, ServiceContext serviceContext, long userId,
			long groupId, long jtbApplicationId) throws PortalException {

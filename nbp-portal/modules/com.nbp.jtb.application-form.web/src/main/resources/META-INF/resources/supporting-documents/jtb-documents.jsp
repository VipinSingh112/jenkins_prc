...
<fieldset class="wizard-fieldset" id="jtbSupportingDocumentFieldset" name="SUPPORTING DOCUMENTS">
	...
	<div class="generic-document">
		...
		<div class="right text-nowrap">
			<input type="file" id="jtbGenericUpload" multiple="multiple"
				name="<portlet:namespace/>jtbGenericUpload"
				hidden="" onchange="attachDetailed(this.id)"/> <label class="custom-upload"
				for="jtbGenericUpload"> <span class="mr-2"> <svg
			...
			<span id="jtbGenericFileChosen"></span>
		</div>
		...
		try{
		supportingDocumentsParentFolderGeneric=GettingDocumentData.getDocumentLibraryParentAndSubFolder(themeDisplay.getScopeGroupId(), jtbApplicationId);
		...
		if(jtbApplicationId>0 && dlFileEntriesGeneric!=null){
			try{
			int docOfJtbAdditionalCounter=1001;
		...
		%>
		<div class="supporting-document-card mb-2 generic-document-list" id="genericSupportingDocuments<%=docOfJtbAdditionalCounter%>">
			<div class="row">
					<div class="col-md-12" id="uploadedFormResult<%=docOfJtbAdditionalCounter%>">
						<%
						%>
						<div class="uploaded-info generic-doc" id="unpload-info-card<%=uploadedFile.getFileEntryId()%>">
							...
							<div class="right">
								...
								<a class="uploaded-delete" href="javascript:;"  onclick="opendelDocPopup('<%=uploadedFile.getFileEntryId()%>','<%=docOfJtbAdditionalCounter%>')">
								...
								<div class="row">
									...
									<div class="col-md-6 mb-3">
										...
										<div class="custom-file">
											<input type="file" class="custom-file-input" id="jtbEnrollDocuments"
												name="<portlet:namespace/>jtbEnrollDocuments" onchange="attachDetailed(this.id)"/>
	...
	<%@ include file="/jtb-wizard-buttons.jsp" %>
</fieldset>
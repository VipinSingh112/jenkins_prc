<fieldset class="wizard-fieldset" id="explosivesTransactionTypeFieldset" name="TYPE OF TRANSACTION">
	<section class="transaction-type" id="transactionType">
        <div class="row">
	            <div class="col-md-12">
	                <p class="common-section-small-heading">Type of Transaction</p>
	            </div>
	        </div>
		<div class="row">
			<div class="col-lg-6">
				<div class="transcation-type-card wizard-card"
					id="explosivesNewCompetency">
					<a href="javascript:;" class="d-block">
						<div class="transaction-type-card-icon text-center mb-3">
							<svg xmlns="http://www.w3.org/2000/svg" width="45" height="45" viewBox="0 0 40 40" fill="none">
								<path d="M5 6.25V33.75C5 34.7446 5.39509 35.6984 6.09835 36.4017C6.80161 37.1049 7.75544 37.5 8.75 37.5H23C22.886 37.4372 22.7754 37.3683 22.6688 37.2937C21.4642 36.496 20.4759 35.4124 19.7921 34.1397C19.1083 32.867 18.7503 31.4448 18.75 30C18.7489 29.3693 18.8181 28.7404 18.9562 28.125H9.375V26.875H19.3313C19.3938 26.7 19.4688 26.5312 19.5438 26.3562C20.1263 25.0927 21.0003 23.9853 22.0937 23.125H9.375V21.875H24.25C24.5974 21.7387 24.9521 21.6218 25.3125 21.525C26.0273 21.3421 26.7622 21.2497 27.5 21.25C27.9184 21.2501 28.3362 21.2815 28.75 21.3438L28.9563 21.375C29.3085 21.434 29.657 21.5133 30 21.6125V11.25H21.875C21.7092 11.25 21.5503 11.1842 21.4331 11.0669C21.3158 10.9497 21.25 10.7908 21.25 10.625V2.5H8.75C7.75544 2.5 6.80161 2.89509 6.09835 3.59835C5.39509 4.30161 5 5.25544 5 6.25ZM9.375 6.875H18.125V8.125H9.375V6.875ZM9.375 11.875H18.125V13.125H9.375V11.875ZM9.375 16.875H25.625V18.125H9.375V16.875Z" fill="#047247"/>
								<path d="M22.5 4.63105V9.9998H27.8688H29.6375L22.5 2.8623V4.63105Z" fill="#047247"/>
								<path d="M28.7502 22.6067C27.7109 22.4303 26.646 22.475 25.6252 22.7379C25.2407 22.8379 24.8646 22.9675 24.5002 23.1254C22.8117 23.8667 21.4532 25.2007 20.6814 26.8754C20.4983 27.2787 20.3499 27.6969 20.2377 28.1254C19.9674 29.171 19.9261 30.2626 20.1167 31.3256C20.3072 32.3885 20.7251 33.3978 21.3416 34.2844C21.9582 35.171 22.759 35.914 23.6892 36.4626C24.6194 37.0112 25.6572 37.3524 26.7314 37.4629C26.9814 37.4629 27.2377 37.5004 27.4939 37.5004C29.262 37.4989 30.9726 36.8728 32.324 35.7326C33.6753 34.5924 34.5803 33.0115 34.8793 31.2689C35.1784 29.5263 34.8521 27.7341 33.9583 26.2086C33.0644 24.6832 31.6603 23.5226 29.9939 22.9317C29.5903 22.785 29.1739 22.6762 28.7502 22.6067ZM31.8752 30.6254H28.1252V34.3754H26.8752V30.6254H23.1252V29.3754H26.8752V25.6254H28.1252V29.3754H31.8752V30.6254Z" fill="#047247"/>
							</svg>
						</div>
						<div class="transaction-type-card-content">
							<p class="transaction-type-card-title wizard-title f2">New
								Competency Certificate</p>
						</div>
					</a>
				</div>
			</div>
			<div class="col-lg-6">
				<div class="transaction-type-card wizard-card" id="explosivesRenew">
					<a href="javascript:;" class="d-block">
						<div class="transaction-type-card-icon text-center mb-3">
							 <svg xmlns="http://www.w3.org/2000/svg" width="45" height="45" viewBox="0 0 40 40" fill="none"> 
                                <path fill-rule="evenodd" clip-rule="evenodd" d="M36.6668 19.9997C36.6668 29.2043 29.2048 36.6663 20.0002 36.6663C10.7954 36.6663 3.3335 29.2043 3.3335 19.9997C3.3335 10.7949 10.7954 3.33301 20.0002 3.33301C29.2048 3.33301 36.6668 10.7949 36.6668 19.9997ZM13.6365 20.1512C13.6365 16.6406 16.4896 13.7876 20.0002 13.7876C21.0713 13.7876 22.0895 14.0527 22.9698 14.53L24.5183 12.9815C23.2138 12.1542 21.6653 11.6663 20.0002 11.6663C15.3123 11.6663 11.5153 15.4633 11.5153 20.1512H8.3335L12.5759 24.3937L16.8183 20.1512H13.6365ZM27.4243 15.9088L23.182 20.1512H26.3638C26.3638 23.6618 23.5108 26.5148 20.0002 26.5148C18.929 26.5148 17.9108 26.2497 17.0305 25.7723L15.482 27.3208C16.7865 28.1482 18.335 28.636 20.0002 28.636C24.688 28.636 28.485 24.839 28.485 20.1512H31.6668L27.4243 15.9088Z" fill="#047247"></path> 
                             </svg>
						</div>
						<div class="transaction-type-card-content">
							<p class="transaction-type-card-title wizard-title f2">Renewal
								Competency Certificate</p>
						</div>
					</a>
				</div>
			</div>
			<input type="hidden" id="explosivesTransactionTypeValue"
					name="explosivesTransactionTypeValue">
		</div>
	</section>
	<section class="" id="competencyCertificateSection"> 
		<hr> 
        	<div class="row"> 
          		<div class="col-md-12"> 
           			<div class="tab-plus-wizard wizard-section"> 
            			<div class="tab-form"> 
             				<div class="row"> 
              					<div class="col-md-12"> 
              						<div class="form-group"> 
						                <p class="tab-form-main-heading">Competency Certificate #</p> 
						                <input type="text" class="form-control" id="competencyCertificateNum" placeholder="Competency Certificate #" value=""/> 
						                <p class="tab-form-sub-heading hide" id="explosiveLicenceNumberValidation" style="color: red;"></p> 
									</div> 
								</div> 
								 	<div class="col-md-6" id="issuedate"> 
              						<div class="form-group"> 
						                <p class="tab-form-main-heading">Date of Issue</p> 
						                <input type="date" class="form-control" id="dateOfIssue" placeholder="Date of Issue" value="" oninput=""/> 
						               <!--  <p class="tab-form-sub-heading hide" id="prospectingLicenceNumberValidation" style="color: red;"></p> --> 
									</div> 
								</div> 
								<div class="col-md-6"  id="datexpiration"> 
              						<div class="form-group"> 
						                <p class="tab-form-main-heading">Date of Expiration</p> 
						                <input type="date" class="form-control" id="dateOfExpiration" placeholder="Date of Expiration" value="" oninput=""/> 
						               <!--  <p class="tab-form-sub-heading hide" id="prospectingLicenceNumberValidation" style="color: red;"></p> --> 
									</div> 
								</div> 
								<div class="col-md-2"> 
 									<button class="btn common-green-bg w-100 wizad w-100" id="explosives-renew-btn" onclick="renewLicense();" type="button">Submit</button> 
      							</div>
     						</div> 
    					</div> 
   					</div> 
  				</div>  
  			</div> 
	</section>
	<%@ include file="/explosives-wizard-buttons.jsp"%>
</fieldset>
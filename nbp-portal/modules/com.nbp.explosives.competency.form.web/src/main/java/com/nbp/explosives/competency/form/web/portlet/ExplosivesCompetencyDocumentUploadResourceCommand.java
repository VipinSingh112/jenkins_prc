...
@Component(immediate = true, property = {
		...
		"mvc.command.name=/explosives/upload/document" 
		}, 
service = MVCResourceCommand.class)
public class ExplosivesCompetencyDocumentUploadResourceCommand implements MVCResourceCommand {
	...
	private static Log _log = LogFactoryUtil.getLog(ExplosivesCompetencyDocumentUploadResourceCommand.class.getName());
	@Override
	public boolean serveResource(ResourceRequest resourceRequest, ResourceResponse resourceResponse)
		...
		try {
			...
			if (Validator.isNotNull(inputStream)) {
				...
				DLFolder dlFolder = createFolder(themeDisplay.getUserId(), explosivesApplicationId,
				...
				ExpandoColumn docNoExpandoColumn = ExpandoColumnLocalServiceUtil.getColumn(expandoTable.getTableId(),
						ExplosivesCompetencyApplicationFormPortletKeys.DOC_NO);
				if (docNoExpandoColumn == null) {
					docNoExpandoColumn = ExpandoColumnLocalServiceUtil.getColumn(themeDisplay.getCompanyId(),
							classNameId, expandoTable.getName(), ExplosivesCompetencyApplicationFormPortletKeys.DOC_NO);
				}

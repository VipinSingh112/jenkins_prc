...
public class ExplosiveApplicationDocumentsCopyUtil {
	private static Log _log = LogFactoryUtil.getLog(ExplosiveApplicationDocumentsCopyUtil.class.getName());
	...
	public synchronized static void copyFolder(long repositoryId, long sourceFolderId, long parentFolderId, String name,
		...
		try {
			destFolder = DLAppServiceUtil.addFolder(String.valueOf(CounterLocalServiceUtil.increment()), repositoryId,
					parentFolderId, String.valueOf(explosivesApplicationId), "Document Folder", serviceContext);
		} catch (PortalException e) {
			_log.error("Error*******" + e.getMessage());
		}

		try {
			copyFolder(sourceDlFolder, destFolder, serviceContext, userId, repositoryId, explosivesApplicationId);
		} catch (PortalException e) {
	...
	protected synchronized static void copyFolder(Folder srcFolder, Folder destFolder, ServiceContext serviceContext, long userId,
			long groupId, long explosivesApplicationId) throws PortalException {

...
@Component(immediate = true, property = {
		...
		"mvc.command.name=/draft/medical/app/url" }, service = MVCResourceCommand.class)
public class MedicalFacilitiesDraftResourceCommand implements MVCResourceCommand {
	private static Log _log = LogFactoryUtil.getLog(MedicalFacilitiesDraftResourceCommand.class.getName());
	@Override
	public boolean serveResource(ResourceRequest resourceRequest, ResourceResponse resourceResponse)
		...
		try {
			medicalFacilitiesApp = MedicalFacilitiesAppLocalServiceUtil.getMedicalFacilitiesApp(medicalApplicationId);
		} catch (Exception e) {
		}
		if (Validator.isNull(medicalFacilitiesApp)) {
			medicalFacilitiesApp = MedicalFacilitiesAppLocalServiceUtil.createMedicalFacilitiesApp(CounterLocalServiceUtil.increment());
		}

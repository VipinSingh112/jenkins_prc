...
@Component
	...
	property = {
		...
		"mvc.command.name=/medical/download/Pdf" 
		}, service = MVCResourceCommand.class
)

public class MedicalFacilitiesDownloadPdfRsourceCommand implements MVCResourceCommand{
	private static Log _log = LogFactoryUtil.getLog(MedicalFacilitiesDownloadPdfRsourceCommand.class);
	...
	public boolean serveResource(ResourceRequest resourceRequest, ResourceResponse resourceResponse)
		...
		try {
			String pdfTemplate = commonsUtil.replaceMedicalFaciltiesPdfVar(medicalApplication.getNatureOfApplicant(),medicalApplication.getMedicalCategory(), medicalApplication.getSubCategory(), 
					medicalApplication.getNursingHomesCategory(),medicalApplicationId, themeDisplay, checkPdfDownloadWithoutDocument);
//		 it converted template into file
		    ...
		    System.out.println("Medical Facitlities-----read pdf");
			String fileNameContentDisp = "MedicalFacilitiesAppPDF.pdf";
			...
			serviceContext = ServiceContextFactory.getInstance(DLFileEntry.class.getName(), resourceRequest);
			  DLFolder dlFolder = createFolder(themeDisplay.getUserId(), medicalApplicationId, themeDisplay.getScopeGroupId()); 
			  DLFolder medicalAppDlFolder=null;
			  medicalAppDlFolder= MedicalFacilitiesApplicationUtil.createSubFolder(themeDisplay.getUserId(), medicalApplicationId, themeDisplay.getScopeGroupId(),
					  "Medical Application Form PDF", "Medical Application Form PDF");
			  System.out.println("folder medical app---->>>");
			...
			if(entry==null) {
			try {
				docFileEntry=	_dlAppService.addFileEntry("MedicalFacilitiesAppPDF" + medicalApplicationId, medicalAppDlFolder.getRepositoryId(),
				...
				dlFileEntry = DLFileEntryLocalServiceUtil.getDLFileEntryByExternalReferenceCode("MedicalFacilitiesAppPDF" + medicalApplicationId,
						medicalApplication.getGroupId());
				docFileEntry = DLAppLocalServiceUtil.getFileEntry(dlFileEntry.getFileEntryId());
				medicalApplication.setPdfFileEntryId(dlFileEntry.getFileEntryId());
				MedicalFacilitiesAppLocalServiceUtil.updateMedicalFacilitiesApp(medicalApplication);
		...
		try {
			...
			if (Validator.isNull(dlFolder)) {
				dlFolder = DLFolderLocalServiceUtil.addFolder(String.valueOf(CounterLocalServiceUtil.increment()), userId, groupId, repositoryId, false,
						rootFolder.getFolderId(), String.valueOf(medicalApplicationId), ROOT_FOLDER_DESCRIPTION, false,
						serviceContext);
			}else {
				 dlFolder=DLFolderLocalServiceUtil.getDLFolder(medicalApplicationId);
			}
		} catch (PortalException e1) {

...
<%
	...
	String documentRequiredSubstring = requiredDocumentArrayList.substring(1,
			requiredDocumentArrayList.length() - 1);
	...
	String documentNotRequiredSubstring = notRequiredDocumentArrayList.substring(1,
			notRequiredDocumentArrayList.length() - 1);
...
<div id="requiredDoc">
	<div class="supporting-document-card mb-3">
				...
				<div class="supporting-document-card-wrapper">
					<div class="left">
						<p class="supporting-document-card-upper-heading"
							id="requiredDoc<%=counterOfPetrol%>">Certificate of Quality</p>
					</div>
					<div class="right text-nowrap">
						<a id="documentUpload<%=counterOfPetrol%>"
							class="custom-upload <%-- <%=isDocumentUploaded%> --%>"
							href="javascript:;"
							onclick="openDocumentUploadPopup('<%=counterOfPetrol%>','Certificate of Quality')">
		...
		<div class="upload-progress progress hidden<%=counterOfPetrol%> hide" >
        	<div id="progress<%=counterOfPetrol%>" class="progress-bar h-100 upload-progress-bar" role="progressbar" aria-valuemin="0" aria-valuemax="100" aria-valuenow="0" ></div>
        </div>
		<div class="row">
			<div class="col-md-12" id="uploadedFormResult<%=counterOfPetrol%>">
				<%
					if (fileEntry != null && fileEntryId != 0) {
				%>
				<div class="uploaded-info" id="unpload-info-card<%=fileEntryId%>">
					<div class="left">
						<p class="uploaded-info-title"
							id="document-name<%=counterOfPetrol%>"><%=fileName%></p>
					...
					<div class="right">
						...
						%>
						<span class="uploaded-info-detail" id="document-issue-date"><%=issueDate%>
						</span>
						<%
							}
								if (Validator.isNotNull(expiryDate)) {
						%>
						<span class="uploaded-info-detail" id="document-expiry-date">
							<%=expiryDate%></span>
						<%
							}
						%>
						...
						<a class="uploaded-actionlink" href="<%=fileUrl%>"
							id="doc-download-btn" target="_blank"> <span class="mr-2">
						...
						</a> <a class="uploaded-delete" href="javascript:;"
							onclick="opendelNcraDocPopup('<%=fileEntryId%>','<%=counterOfPetrol%>')">
				...
				<div class="modal fade commonsavepopup"
					id="deletepopup<%=fileEntryId%>" data-keyboard="false"
					tabindex="-1" aria-labelledby="staticBackdropLabel"
					aria-hidden="true" style="display: none;">
					<div
						class="modal-dialog modal-dialog-centered my-0 modal-dialog-sm">
						<div class="modal-content">
							...
							<div class="modal-body">
								<div class="row">
									<div class="col-md-12">
										<p class="modal-title f1 mb-4">Are you sure you want to
											Delete?</p>
									</div>
								</div>
							</div>
							<div class="modal-footer border-0 d-flex justify-content-end">
								<a href="javascript:;"
									...
									onClick="deleteDocument('<%=fileEntryId%>');"> Yes<span
									class="ml-2"> </span>
								</a> <a href="javascript:;"
									class="common-yellow-bg text-dark common-nav-links text-white popup-dashboard"
									data-dismiss="modal" aria-label="Close"> No<span
									class="ml-2"> </span>
								</a>
							</div>
						</div>

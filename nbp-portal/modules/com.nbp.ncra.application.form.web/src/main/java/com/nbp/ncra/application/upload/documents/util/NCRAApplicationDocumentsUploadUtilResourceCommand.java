...
@Component(immediate = true, property = {
		...
		"mvc.command.name=/doc/upload/in/ncra/form/url"
		},
		service = MVCResourceCommand.class
		)
public class NCRAApplicationDocumentsUploadUtilResourceCommand implements MVCResourceCommand {
	private static long PARENT_FOLDER_ID = DLFolderConstants.DEFAULT_PARENT_FOLDER_ID;
	private static String ROOT_FOLDER_NAME = "NCRA Supported Documents";
	private static Log _log = LogFactoryUtil.getLog(NCRAApplicationDocumentsUploadUtilResourceCommand.class);
@Override
	...
	_log.info("File upload started-----------------------");
		...
		System.out.println("parseLongCheck in generic Docment command -------------"+parseLongCheck);
		System.out.println("ncraApplicationId in generic resource command  -------------"+ncraApplicationId);
		_log.info("File name-----------------------"+folderName);
	...
	if (folderName.equals("Generic Documents")) {
		...
		if (Validator.isNotNull(fileList) && fileList.length > 0) {
			dlFolder = NCRAApplicationUtil.createSubFolder(
					themeDisplay.getUserId(), ncraApplicationId, themeDisplay.getScopeGroupId(), folderName,
			...
			for (File fileItem : fileList) {
				try {
					...
					if (Validator.isNotNull(fileItem) && Validator.isNotNull(fileName)) {
						...
						System.out.println("JSON Array in documents resource Command----"+jsonArrayWithUrl);
					}
				} catch (Exception e) {
				}
				counter = counter + 1;

...
public class NcraApplicationDocumentsCopyUtil {
	private static Log _log = LogFactoryUtil.getLog(NcraApplicationDocumentsCopyUtil.class.getName());

	public static void copyFolder(long repositoryId, long sourceFolderId, long parentFolderId, String name,
			String description, ServiceContext serviceContext, long folderId, long userId, long ncraApplicationId) {
		...
		try {
			destFolder = DLAppServiceUtil.addFolder(String.valueOf(CounterLocalServiceUtil.increment()), repositoryId, parentFolderId, String.valueOf(ncraApplicationId),
					"Document Folder", serviceContext);
		} catch (PortalException e) {
			_log.error("Error*******" + e.getMessage());
		}

		try {
			copyFolder(sourceDlFolder, destFolder, serviceContext, userId, repositoryId, ncraApplicationId);
		} catch (PortalException e) {
	...
	protected static void copyFolder(Folder srcFolder, Folder destFolder, ServiceContext serviceContext, long userId,
			long groupId, long ncraApplicationId) throws PortalException {
		...
		for (Folder srcSubfolder : srcSubfolders) {
			Folder destSubfolder = DLAppServiceUtil.addFolder(
					...
					String.valueOf(CounterLocalServiceUtil.increment()), srcSubfolder.getGroupId(), curDestFolder.getFolderId(), srcSubfolder.getName(),
	...
	public static DLFileEntry copyFileEntry(long userId, long groupId, long repositoryId, long fileEntryId,
		...
		try {
			newDLFileEntry = DLFileEntryLocalServiceUtil.addFileEntry(null, userId, groupId, repositoryId, destFolderId,
					sourceFileName, dlFileEntry.getMimeType(), title, title, dlFileEntry.getDescription(), null,
					dlFileEntry.getFileEntryTypeId(), null, null, inputStream, dlFileEntry.getSize(),

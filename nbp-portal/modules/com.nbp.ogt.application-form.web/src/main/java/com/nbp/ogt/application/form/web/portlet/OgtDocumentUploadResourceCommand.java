...
@Component(immediate = true, property = {
		...
		"mvc.command.name=/ogt/document/upload" }, service = MVCResourceCommand.class)

public class OgtDocumentUploadResourceCommand implements MVCResourceCommand {
	...
	private static Log _log = LogFactoryUtil.getLog(OgtDocumentUploadResourceCommand.class.getName());
	...
	public boolean serveResource(ResourceRequest resourceRequest, ResourceResponse resourceResponse)
		...
		try {
			...
			if (Validator.isNotNull(inputStream)) {
				...
				DLFolder dlFolder = createFolder(themeDisplay.getUserId(), ogtApplicationId,
				...
				ExpandoColumn docNoExpandoColumn = ExpandoColumnLocalServiceUtil.getColumn(expandoTable.getTableId(),
						OgtApplicationFormWebPortletKeys.DOC_NO);
				if (docNoExpandoColumn == null) {
					docNoExpandoColumn = ExpandoColumnLocalServiceUtil.getColumn(themeDisplay.getCompanyId(),
							classNameId, expandoTable.getName(), OgtApplicationFormWebPortletKeys.DOC_NO);
				}
		...
		try {
			...
			if (Validator.isNull(dlFolder)) {
				dlFolder = DLFolderLocalServiceUtil.addFolder(String.valueOf(CounterLocalServiceUtil.increment()), userId, groupId, repositoryId, false,
						rootFolder.getFolderId(), String.valueOf(ogtApplicationId), ROOT_FOLDER_DESCRIPTION,

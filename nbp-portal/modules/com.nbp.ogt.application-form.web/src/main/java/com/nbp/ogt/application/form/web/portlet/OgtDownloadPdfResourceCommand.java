...
import com.nbp.ogt.application.util.OgtApplicationUtil;
...
property = {
	...
	"mvc.command.name=/ogt/download/pdf" 
	}, service = MVCResourceCommand.class
)

public class OgtDownloadPdfResourceCommand implements MVCResourceCommand{
	private static Log _log = LogFactoryUtil.getLog(OgtDownloadPdfResourceCommand.class);
	private static long PARENT_FOLDER_ID = DLFolderConstants.DEFAULT_PARENT_FOLDER_ID;
	private static String ROOT_FOLDER_NAME = "OGT Supported Documents";
	private static String ROOT_FOLDER_DESCRIPTION = "OGT Supported Documents";
	...
	public boolean serveResource(ResourceRequest resourceRequest, ResourceResponse resourceResponse)
		...
		try {
			String pdfTemplate = commonsUtil.replaceOgtApplicationPdfVar(ogtApplication.getTypeOfMatter(), ogtApplicationId, themeDisplay, checkPdfDownloadWithoutDocument);
//		 it converted template into file
			...
			String fileNameContentDisp = "OgtApplicationPDF.pdf";
			...
			serviceContext = ServiceContextFactory.getInstance(DLFileEntry.class.getName(), resourceRequest);
			  DLFolder dlFolder = createFolder(themeDisplay.getUserId(), ogtApplicationId, themeDisplay.getScopeGroupId()); 
			  DLFolder ogtAppDlFolder=null;
			  ogtAppDlFolder= OgtApplicationUtil.createSubFolder(themeDisplay.getUserId(), ogtApplicationId, themeDisplay.getScopeGroupId(),
					  "OGT Application Form PDF", "OGT Application Form PDF");
			commonsUtil.updateFolderPermission(ogtAppDlFolder);
			...
			try {
			entry=DLAppLocalServiceUtil.getFileEntryByFileName(themeDisplay.getScopeGroupId(), ogtAppDlFolder.getFolderId(), fileNameContentDisp);
			...
			if(entry==null) {
			try {
				docFileEntry=	_dlAppService.addFileEntry("OgtApplicationPDF" + ogtApplicationId, ogtAppDlFolder.getRepositoryId(),
						ogtAppDlFolder.getFolderId(), fileNameContentDisp, mimeType, title, null, StringPool.BLANK,
				...
				dlFileEntry = DLFileEntryLocalServiceUtil.getDLFileEntryByExternalReferenceCode("OgtApplicationPDF" + ogtApplicationId,
						ogtApplication.getGroupId());
				docFileEntry = DLAppLocalServiceUtil.getFileEntry(dlFileEntry.getFileEntryId());
				ogtApplication.setPdfFileEntryId(dlFileEntry.getFileEntryId());
	...
	private DLFolder createFolder(long userId, long ogtApplicationId, long groupId) {
		...
		try {
			...
			try {
				dlFolder = DLFolderLocalServiceUtil.getFolder(repositoryId, rootFolder.getFolderId(),
						String.valueOf(ogtApplicationId));
			} catch (Exception e) {
				e.printStackTrace();
			}
			if (Validator.isNull(dlFolder)) {
				dlFolder = DLFolderLocalServiceUtil.addFolder(String.valueOf(CounterLocalServiceUtil.increment()), userId, groupId, repositoryId, false,
						rootFolder.getFolderId(), String.valueOf(ogtApplicationId), ROOT_FOLDER_DESCRIPTION, false,
						serviceContext);
			}else {
				 dlFolder=DLFolderLocalServiceUtil.getDLFolder(ogtApplicationId);
			}
		} catch (PortalException e1) {

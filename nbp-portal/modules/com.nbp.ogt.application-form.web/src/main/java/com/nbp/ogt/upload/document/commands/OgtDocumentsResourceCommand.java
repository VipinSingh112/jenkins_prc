package com.nbp.ogt.upload.document.commands;
...
@Component(immediate = true, property = {
		...
		"mvc.command.name=/ogt/signature/upload"
		},
		service = MVCResourceCommand.class 
		)
public class OgtDocumentsResourceCommand implements MVCResourceCommand {
	private static long PARENT_FOLDER_ID = DLFolderConstants.DEFAULT_PARENT_FOLDER_ID;
	private static String ROOT_FOLDER_NAME = "OGT Supported Documents";
	private static Log _log = LogFactoryUtil.getLog(OgtDocumentsResourceCommand.class);
@Override
	...
	if (folderName.equals("Generic Documents")) {
		...
		if (Validator.isNotNull(fileList) && fileList.length > 0) {
			dlFolder = com.nbp.ogt.application.util.OgtApplicationUtil.createSubFolder(
					themeDisplay.getUserId(), ogtApplicationId, themeDisplay.getScopeGroupId(), folderName,
					"OGT Supported Documents");
				
				  if(folderName.equals("Signature of Person Completing Form")||folderName.equals("Signature of Justice Peace")||folderName.equals("Signature of Voting Trustee's")
						  ||folderName.equals("Signature of Payment Trustee's")) { 
					  try {
